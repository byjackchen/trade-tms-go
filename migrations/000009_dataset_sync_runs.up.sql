...
--
-- tms.dataset_sync (000002) stays the watermark table (one row per dataset,
-- last wall-clock sync + cumulative row count). This table is additive
-- [IMPROVE]: one row per dataset per sync/bootstrap run with
-- started/finished timestamps, net-new row count, status and error text, so
-- operators can audit catchup history and partial failures.
...
COMMENT ON TABLE tms.dataset_sync_runs IS
    'Append-only audit of Sharadar sync runs: one row per dataset per run. rows_added = net-new keys (added semantics, spec data-sharadar.md §6); status=error keeps the warn-and-continue failure text (spec §8).';
