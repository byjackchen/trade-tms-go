...
// and audit trail cannot be faked meaningfully). It uses the same ephemeral
// docker-Postgres harness pattern as internal/jobs: a single timescaledb
// container per test binary on a random loopback port, skipped when docker is
// unavailable (TMS_TEST_NO_DOCKER=1). Migrations apply the full schema.
...
func TestMain(m *testing.M) {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "hyperopt/study: integration tests will skip: %v\n", err)
		os.Exit(m.Run())
	}
	defer cleanup()
	os.Exit(m.Run())
...
func requirePG(t *testing.T) *pgxpool.Pool {
	t.Helper()
	if testPool == nil {
		t.Skipf("postgres harness unavailable: %s", pgUnavailable)
	}
	return testPool
}
...
// ---------------------------------------------------------------------------
// ephemeral docker-Postgres harness (mirrors internal/jobs/pgtest_test.go).
// ---------------------------------------------------------------------------
...
func startEphemeralPG() (func(), error) {
	...
	name := fmt.Sprintf("tmsgo-hyperopt-test-%d", time.Now().UnixNano())
	...
	out, err := exec.CommandContext(runCtx, "docker", "run", "-d", "--rm",
		...
		"-p", "127.0.0.1:0:5432", testPGImage).CombinedOutput()
	if err != nil {
...
func mappedPort(container string) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("parsing docker port %q: %w", line, err)
	}
	var port int
