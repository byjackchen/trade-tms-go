...
// Portfolio facade. Zero engine dependency — strategies build a ProposedOrder +
// AccountSnapshot per check and receive a RiskDecision.
//
...
// AccountSnapshot is a read-only view of account state at a point in time
...
func (a AccountSnapshot) TotalPnLToday() dec {
...
func (a AccountSnapshot) StrategyPosition(strategyID, symbol string) int64 {
	if a.Positions == nil {
		return 0
	}
...
func (a AccountSnapshot) NetPositionAcrossStrategies(symbol string) int64 {
	var net int64
...
// not rounded.
func (a AccountSnapshot) GrossExposureForStrategy(strategyID string) dec {
	total := decZero()
	for k, qty := range a.Positions {
