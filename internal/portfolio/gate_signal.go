...
// Both callers built a ProposedOrder, ran Portfolio.Check, and on a rejection
...
func GateSignal(gate *Portfolio, proposed ProposedOrder, account AccountSnapshot, price domain.Price, rec RejectionRecorder) RiskDecision {
	if gate == nil {
		return Approve()
	}
