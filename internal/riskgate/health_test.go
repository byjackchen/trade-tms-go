...
func healthPortfolio(t *testing.T, haltPct float64) *Gate {
	t.Helper()
...
func TestHealthSnapshotHandDerived(t *testing.T) {
	...
	atBoundary := pf.HealthSnapshot(PortfolioSnapshot{
	...
	below := pf.HealthSnapshot(PortfolioSnapshot{
	...
	hr := pf.HealthSnapshot(PortfolioSnapshot{
	...
	conc := pf.HealthSnapshot(PortfolioSnapshot{
		NAV: MustDec("200000"),
		Positions: map[PositionKey]int64{
	...
	zeroNav := pf.HealthSnapshot(PortfolioSnapshot{NAV: decZero(), RealizedPnLToday: MustDec("100")})
