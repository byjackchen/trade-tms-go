...
// allocator.go (spec §5): per-strategy capital-budget enforcement. Each strategy
// gets a fixed fraction of NAV; a proposed order is rejected if it would push
// that strategy's GROSS dollar exposure beyond its budget.
...
// StrategyAllocation is one row of the allocation table: a strategy id and its
// capital fraction in (0, 1].
type StrategyAllocation struct {
	StrategyID string
	CapitalPct float64
}

// Allocator enforces per-strategy capital budgets.
type Allocator struct {
	table map[string]float64
}

// NewAllocator validates and builds an Allocator:
...
// nav * pct (exact).
func (a *Allocator) budgetDollars(strategyID string, account PortfolioSnapshot) dec {
...
// exposure over budget. FLAT and qty<=0 always approve.
// Rule names: "allocator.unregistered_strategy" (no/zero budget),
// "allocator.budget_exceeded" (strict >). The budget comparison is exact.
func (a *Allocator) CheckOrderWithinBudget(order ProposedOrder, account PortfolioSnapshot) RiskDecision {
