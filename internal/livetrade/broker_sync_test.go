...
// EXTERNAL book by SyncFromBroker, and reconciliation captures it.
func TestSyncReflectsExternalBrokerPosition(t *testing.T) {
	h := newPaperSync(t, 100000)
	...
	// Before the sync the EXTERNAL book knows nothing about TSLA.
	_, ok := h.account.Position(livetrade.ExternalStrategyID, "TSLA")
	assert.False(t, ok, "EXTERNAL book must not know the external position before sync")

	rep, err := h.mc.SyncFromBroker(ctx, "alice")
	require.NoError(t, err)

	// The pull observed the external position; the reflection moved the EXTERNAL book.
	assert.Equal(t, 1, rep.PositionsObserved)
	assert.Equal(t, 1, rep.Reflected, "the external position was reflected")

	// The EXTERNAL book now mirrors the broker truth (long 25 TSLA).
	pos, ok := h.account.Position(livetrade.ExternalStrategyID, "TSLA")
	require.True(t, ok, "the external position is now reflected in the EXTERNAL book")
	assert.Equal(t, domain.Qty(25), pos.SignedQty)

	// Reconciliation ran + was persisted. With the broker truth reflected into the
	// EXTERNAL book, the broker net == the book net => the symbol MATCHES (no drift).
...
// TestSyncReconciliationCapturesDriftVsStrategyBooks: when the EXTERNAL book is NOT
...
func TestSyncIsIdempotent(t *testing.T) {
	...
	pos1, _ := h.account.Position(livetrade.ExternalStrategyID, "AMD")
	...
	pos2, _ := h.account.Position(livetrade.ExternalStrategyID, "AMD")
...
func TestSyncTracksIncrementalExternalTrades(t *testing.T) {
	...
	require.NoError(t, err)
	pos, _ := h.account.Position(livetrade.ExternalStrategyID, "AAPL")
...
// prior sync; the broker no longer reports it, so the sync drives the EXTERNAL book
// back to flat.
func TestSyncReflectsExternalClose(t *testing.T) {
	...
	require.NoError(t, err)
	pos, _ := h.account.Position(livetrade.ExternalStrategyID, "MSFT")
	...
	pos, _ = h.account.Position(livetrade.ExternalStrategyID, "MSFT")
	assert.Equal(t, domain.Qty(0), pos.SignedQty, "the EXTERNAL book is flat after the external close")
}
...
func TestSyncIsReadOnly(t *testing.T) {
	...
	before, _ := h.venue.GetOrderList(ctx, syncPaperAcc, 0)
	_, err := h.mc.SyncFromBroker(ctx, "frank")
	require.NoError(t, err)
	after, _ := h.venue.GetOrderList(ctx, syncPaperAcc, 0)
