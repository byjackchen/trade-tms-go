...
// manual_sync.go is DIRECTION 2 — the broker -> TMS SYNC/REFLECT flow on the
// MANUAL desk (the user's PRIMARY case). The operator trades DIRECTLY in the moomoo
// app (NO order placed via TMS); they then return to TMS and click "Sync from
// broker". SyncFromBroker pulls the account's ACTUAL state (Trd_GetPositionList +
// Trd_GetOrderList + Trd_GetOrderFillList + Trd_GetFunds) and REFLECTS it into TMS
// so positions/orders/fills/account show what was done in moomoo.
//
...
// signal-mode operator can sync to see/manage what they actually hold). Per-order
// confirmation + the risk gate are IRRELEVANT here precisely because nothing crosses
// the wire to the venue.
//
// ATTRIBUTION: the synced broker positions are reflected under the MANUAL pseudo-
...
// MANUAL book net per symbol, so re-syncing the SAME broker state reflects nothing
...
type SyncReport struct {
	...
	OrdersObserved    int
	FillsObserved     int
	// Reflected is how many symbols' MANUAL/EXTERNAL book net was moved to match the
...
// (READ-ONLY) and REFLECTS it into TMS under the MANUAL/EXTERNAL book, then runs the
...
//
// Flow:
//  1. reflect the broker truth into the MANUAL book (SyncBrokerInto — read-only pull
...
func (m *ManualController) SyncFromBroker(ctx context.Context, operator string) (SyncReport, error) {
	if strings.TrimSpace(operator) == "" {
		return SyncReport{}, fmt.Errorf("%w: sync requires an operator", domain.ErrInvalidArgument)
	}
	ts := m.clock()

	// (1) read-only pull + reflect into the MANUAL/EXTERNAL book. NO order is placed.
	syncRes, err := m.exec.SyncBrokerInto(ctx, ManualStrategyID)
	if err != nil {
		...
		return SyncReport{}, fmt.Errorf("manual sync: %w", err)
	}
	...
	if m.reconciler != nil {
		...
		if rerr != nil {
			return report, fmt.Errorf("manual sync: reconcile: %w", rerr)
		}
