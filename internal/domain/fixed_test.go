...
func TestParseFixed4HalfEven(t *testing.T) {
	// Half-to-even (Python decimal ROUND_HALF_EVEN), NOT half-up: ties go to
	// the even neighbor. The half-up answers are noted where they differ.
	tests := []struct {
		in   string
		want int64
	}{
...
func TestFormatFixed4Canonical(t *testing.T) {
	tests := []struct {
		...
		{-10000, "-1"},
		{1234500, "123.45"},
		{1234000, "123.4"}, // canonical trims trailing zeros (vs Python Decimal "123.40"; use StringFixed(2) for display parity)
		{705000, "70.5"},
		{12345, "1.2345"},
...
func TestFormatFixedDP(t *testing.T) {
	tests := []struct {
		...
		{1234500, 2, "123.45"},
		{1234000, 2, "123.40"}, // Python Decimal str parity for 2dp prices
		{1234500, 4, "123.4500"},
...
func TestPyRoundHandcrafted(t *testing.T) {
	// CPython round(float, ndigits): correctly-rounded half-to-even on the
...
// TestGoldenCPython validates PyRound (ndigits 0, 2, 4) and the
// Decimal(str(x)) half-even quantization path of *FromFloat64 against
// vectors generated by the reference CPython virtualenv
// (testdata/gen_golden.py).
func TestGoldenCPython(t *testing.T) {
	...
	for sc.Scan() {
		...
		for i, n := range []int{0, 2, 4} {
			...
			if math.Float64bits(got) != math.Float64bits(want) {
				t.Errorf("line %d: PyRound(%v, %d) = %v (%x), CPython round = %v (%x)",
		...
		if fields[4] == "OOR" {
			...
			if int64(gotP) != want {
				t.Errorf("line %d: PriceFromFloat64(%v) = %d, CPython Decimal(str).quantize = %d",
...
func TestFixed4ToFloatSingleRounding(t *testing.T) {
	...
	// path must stay correctly rounded (Python float(Decimal) parity).
