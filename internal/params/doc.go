...
//
// Responsibilities, in three layers:
//
...
//     with the precedence (db -> env-dir -> baseline) over the P0 DB schema:
//
...
//     instead of failing (spec strategy-sepa.md §1.4).
//
...
//     struct, and runs the config's runtime validation (bounds such as
//     risk_pct in (0,100], exit_z < entry_z, top_k in [1,len(universe)],
//     eod_exit_time HH:MM, IANA timezone, etc.).
//
// All numeric values are float64 / int64 (locked decision 4).
package params
