...
// signals into market orders (spec §10): LONG -> BUY target_qty; FLAT -> reverse
// the live net position to flat (no-op when already flat); SHORT unsupported.
//
// This package — NOT the pure sepa package — is the only place that imports
// engine/domain, preserving the layering constraint (spec §0): the core SEPA
// package never imports broker/engine code.
//
