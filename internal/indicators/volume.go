...
// denominator (sepa/signal.py:256-273 _breakout_volume_ok, which mirrors
// trade-minervini's `klines.iloc[-(base+1):-1].mean()`).
//
...
// BreakoutVolumeOK replicates the full SEPA breakout-volume gate
// (sepa/signal.py:256-273): with base_lookback fixed at 60 in the reference,
// today's volume must strictly exceed `multiple` * the trailing-`base` baseline
// (excluding today). Returns false during warmup or when the baseline is <= 0,
// matching the reference's `base_avg_vol <= 0` guard.
