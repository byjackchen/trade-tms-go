...
// strategies (SEPA, Pairs, Sector Rotation, Intraday ORB). Every primitive is
// ported from the Python reference (strategies/sepa/_indicators.py, _swing.py,
// vcp.py, stage.py, trend_template.py and the Pairs OLS/z-score path) with
// pandas/numpy/statistics-exact semantics: identical warm-up/NaN handling,
// rolling-window min_periods == window, ddof conventions, and round-half-even
// output rounding. Golden tests (golden_test.go + testdata/golden.json,
// regenerated by tmp/indicators_parity/gen.py against the upstream venv) assert
// parity within 1e-9.
//
// Surface:
//
...
//     (look-ahead guard).
//   - round.go        RoundHalfEven (CPython banker's rounding).
//
...
// implementations here are stdlib-only for exact pandas parity.
package indicators
