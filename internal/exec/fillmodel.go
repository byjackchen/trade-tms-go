...
// Two profiles ship:
//
//   - nautilus-compat (the parity target): discovered empirically by running
//     the reference Nautilus BacktestEngine (TestInstrumentProvider.equity,
//     AccountType.MARGIN, OmsType.NETTING, default FillModel, leverage 10,
//     bar_execution with bar_adaptive_high_low_ordering=False). A market order
//     submitted in on_bar(T) fills within bar T (same ts) against the bar's
//     decomposed LAST-price ticks. The matching engine posts the bar's CLOSE
//     tick with the depth `close_tick_vol = compute_bar_quarter_sizes(volume)`
//     (quarter = max(volume//4, 1); close = (3*quarter >= volume) ? 1 :
//     volume - 3*quarter — data.pxd, see closeTickVolume below). The
...
//     single level, so the rest fills at exactly one increment away. Verified
//     across qty/volume/side permutations (tmp/parity/probe_depthwalk.py):
...
//     the zero-cost parity GATE profile.
//
...
// PriceIncrement is the equity price increment (tick size): 0.01, matching the
// reference instrument (price_precision=2, price_increment=0.01, §7.2). It is
// the increment the nautilus-compat model walks residual quantity by.
...
const (
	// FillThisBar fills against the bar currently being processed (nautilus-
	// compat: same bar as submission).
...
// NautilusCompatModel is the zero-cost parity profile: fill against the bar's
// close-tick depth at the close price, residual one increment adverse, same
// bar, no commission.
type NautilusCompatModel struct{}

// Name returns "nautilus-compat".
func (NautilusCompatModel) Name() string { return "nautilus-compat" }

// Timing returns FillThisBar.
func (NautilusCompatModel) Timing() FillTiming { return FillThisBar }

// Fill returns the close-tick depth at the bar close plus a residual leg one
// price increment adverse to the side, reproducing the empirically observed
// Nautilus matching. Non-market orders are rejected (the parity profile models
// the market orders the reference emits).
func (NautilusCompatModel) Fill(order domain.Order, bar domain.Bar) ([]FillLeg, error) {
	if order.Type != domain.OrderTypeMarket {
		return nil, fmt.Errorf("%w: nautilus-compat fills MARKET orders only, got %s",
			domain.ErrInvalidArgument, order.Type)
	}
	if order.Qty <= 0 {
		return nil, fmt.Errorf("%w: nautilus-compat order %s non-positive qty %d",
...
func (NautilusCompatModel) Commission(domain.Qty, domain.Price) (domain.Money, error) {
	return 0, nil
}

// closeTickVolume returns the volume Nautilus assigns to the bar's CLOSE tick
// when it decomposes a bar into O/H/L/C ticks for matching. It reproduces
// compute_bar_quarter_sizes (nautilus_trader/model/data.pxd) EXACTLY, with
// min_size = 1 share (equity quantity_precision = 0):
//
//	quarter = volume // 4
...
// volume=3 gives 3 naively but Nautilus assigns 1 (quarter floors to 1, and
// 3*1 >= 3 trips the underflow guard). Verified empirically against the
// BacktestEngine across volume/qty/side permutations (tmp/parity/probe_depthwalk
// .py; golden table tmp/parity/nautilus_out/depthwalk.json). [MUST-MATCH]
...
type RealisticModel struct {
	...
	// lower, by SlippageBps/10000 of the reference (next-open) price.
	SlippageBps float64
