...
// vendored Python moomoo SDK produces for the same request (same field values,
// same serial number). The reference bytes are generated by
// tmp/conformance/dump_frames.py driving the SDK's own pack_pb_req (so the
// header layout, SHA-1, and struct.pack are the SDK's, not a reimplementation).
//
// testdata/conformance_frames.json is committed; to regenerate after a proto
// change, run (from the trade-multi-strategies venv):
//
//	.venv/bin/python tmp/conformance/dump_frames.py \
//	  > internal/adapters/moomoo/testdata/conformance_frames.json
//
// Each case here MUST build the identical protobuf message the harness builds.
...
// TestEncodeFrameMatchesPythonSDK is the conformance gate: identical bytes.
func TestEncodeFrameMatchesPythonSDK(t *testing.T) {
