...
//	  "strategy":         "scripted",           // only "scripted" for now
//	  "intents": [ {"date":"YYYY-MM-DD","ticker":"AAPL","side":"LONG","qty":100}, ... ],
...
func (h *Backtest) Run(ctx context.Context, job *jobs.Job, report jobs.ProgressFn) (any, error) {
	...
	cfg, runTS, err := h.buildConfig(ctx, p)
	if err != nil {
		return nil, err
	}
...
// the run_ts (idempotency key).
func (h *Backtest) buildConfig(ctx context.Context, p backtestParams) (engine.Config, string, error) {
	if p.Strategy != "" && p.Strategy != "scripted" {
		return engine.Config{}, "", fmt.Errorf("backtest.run: unsupported strategy %q (only \"scripted\" is implemented)", p.Strategy)
	}
	start, err := calendar.ParseDate(p.Start)
	if err != nil {
		return engine.Config{}, "", fmt.Errorf("backtest.run: invalid start %q (want YYYY-MM-DD): %w", p.Start, err)
	}
	end, err := calendar.ParseDate(p.End)
	if err != nil {
		return engine.Config{}, "", fmt.Errorf("backtest.run: invalid end %q (want YYYY-MM-DD): %w", p.End, err)
	}
	...
	if len(tickers) == 0 {
		return engine.Config{}, "", errors.New("backtest.run: no tickers resolved (supply \"tickers\" or a \"universe\" window)")
	}

	startBal := domain.Money(0)
	if p.StartingBalance != nil {
		startBal, err = domain.MoneyFromFloat64(*p.StartingBalance)
		if err != nil {
			return engine.Config{}, "", fmt.Errorf("backtest.run: invalid starting_balance: %w", err)
		}
	} else {
	...
	if !profile.IsValid() {
		return engine.Config{}, "", fmt.Errorf("backtest.run: unknown fill_profile %q (want \"nautilus-compat\" or \"realistic\")", p.FillProfile)
	}

	intents, err := buildIntents(p.Intents)
	if err != nil {
		return engine.Config{}, "", err
	}
	...
	if p.Realistic != nil {
		...
		if cerr != nil {
			return engine.Config{}, "", fmt.Errorf("backtest.run: invalid commission_per_share: %w", cerr)
		}
