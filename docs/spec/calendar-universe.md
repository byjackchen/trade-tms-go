...
Scope: trading-calendar/session handling, screener protocol, SEPA screener
ranking, market-cap top-N capping, `TMS_LIVE_UNIVERSE_LIMIT` logic, and the
exact ticker-exclusion sets.
...
- **[MUST-MATCH]** — the Go implementation must replicate this behavior
  byte-/semantics-equivalently, including edge cases, NaN handling, rounding,
  and ordering.
- **[IMPROVE]** — known weakness in the original. Both the original behavior
  and the proposed Go improvement are documented; the Go side may implement
  the improvement, but the original behavior MUST remain available (or the
  difference must be explicitly surfaced) so reference-output comparison stays
  possible.
...
**[MUST-MATCH]** The reference system deliberately does **not** depend on any
...
The decision is documented in `src/data/sharadar_cache/catchup.py:55-58`:
...
**[IMPROVE]** Original: holidays and half-days are not modeled anywhere (see
...
(`src/adapters/moomoo/daily_aggregator.py:37-39`) and reproducing reference
outputs requires the same drop.

### 1.2 Catch-up "trading days" = pandas business days (Mon–Fri)

Source: `src/data/sharadar_cache/catchup.py:52-65`; tests
`tests/data/sharadar_cache/test_catchup.py:57-64`.

**[MUST-MATCH]** `_trading_days(start, end)`:

- Returns every **weekday** (Mon–Fri) from `start` **inclusive** to `end`
  **inclusive** (`pd.bdate_range(start, end, freq="B")`,
  `catchup.py:62-65`).
- If `end < start` → empty list (`catchup.py:60-61`).
- If `start` or `end` falls on a weekend, `bdate_range` simply excludes
  ...
  `[Mon 2026-05-25, Tue 2026-05-26]`, `test_catchup.py:57-61`).
...
**[MUST-MATCH]** Catch-up window derivation (`catchup.py:103-124`):
...
- Source of truth for "last synced" = `meta.last_sync["SEP"]` (a timestamp);
  ...
  (`catchup.py:103-112`).
- `today_utc = datetime.now(tz=UTC).date()` unless injected
  (`catchup.py:68-69,114`). **Timezone: UTC**, not exchange-local.
- `target = today_utc - 1 day` (T-1) (`catchup.py:115`).
- `start = last_sep.date()` — i.e. the already-synced day is **re-fetched**
  (inclusive start). Idempotent: same-day re-writes add 0 rows
  (`catchup.py:116,91-94`).
- Empty day list → "cache fresh" no-op report (`catchup.py:118-124`).

**[MUST-MATCH]** Per-day loop order and failure policy (`catchup.py:140-216`):
for each day, SEP first then SFP (`catchup.py:137-158`); each step wrapped in
try/except — a failure appends to `errors` and continues (warn-and-continue,
never aborts startup, `catchup.py:8-11`); a day counts as `succeeded` only if
both SEP and SFP succeeded (`catchup.py:141-161`); meta is persisted after
every day (`catchup.py:163-164`); then one TICKERS refresh, then SF1 + EVENTS
keyed on the refreshed ticker list (`catchup.py:166-202`); empty ticker list
skips SF1/EVENTS with an error entry (`catchup.py:177-180`).

### 1.3 Live RTH session window (minute → daily aggregation)

Source: `src/adapters/moomoo/daily_aggregator.py`.

**[MUST-MATCH]** Session definition:

| Item | Value | Source |
|---|---|---|
| Timezone | `America/New_York` (IANA, DST-aware) | `daily_aggregator.py:51` |
| RTH open | 09:30 ET, minute-of-day 570, **inclusive** | `daily_aggregator.py:52` |
| RTH close | 16:00 ET, minute-of-day 960, **inclusive** (the 16:00 minute bar is kept) | `daily_aggregator.py:53` |
| Membership test | `570 <= et.hour*60 + et.minute <= 960` on the bar's `ts_event` converted from ns-UTC to ET | `daily_aggregator.py:56-64` |

- Pre-market and post-market bars are silently dropped
  (`daily_aggregator.py:20-22,102-103`).
- **Half-days are NOT handled**: on 13:00-ET-close days, bars after 13:00 are
  still accepted until 16:00 — explicit out-of-scope note at
  `daily_aggregator.py:37-38`. (See [IMPROVE] in §1.1.)
- **Holiday detection is NOT implemented**: a non-trading day simply produces
  zero RTH bars and no daily emit (`daily_aggregator.py:39,124-125`).

**[MUST-MATCH]** Daily-bar assembly semantics (`daily_aggregator.py:96-140`):
...
- Minute pushes dedup by exact `ts_event` (nanoseconds): a dict keyed on
  ...
  (`daily_aggregator.py:89-92,115-116`).
- Day-boundary trigger: when an RTH minute bar arrives whose ET **date**
  ...
  emitted and state resets (`daily_aggregator.py:105-113`). First-ever bar
  just records the date. Consequence: strategies see day D's daily bar at
  the start of day D+1 (`daily_aggregator.py:24-28`).
- Emitted bar: bars sorted by `ts_event` ascending; `open` = first bar's
  ...
  (`daily_aggregator.py:126-139`).
- If a day accumulated zero RTH bars, **no** emit on rollover
  (`daily_aggregator.py:118-125`).

### 1.4 Pre-open trigger (screener refresh scheduling)

Source: `src/strategies/sepa/universe_runner.py:199-218,320-333`.

**[MUST-MATCH]** Day-boundary detection in `SEPAUniverseRunner`:

- `_maybe_trigger_pre_open` runs on **every** bar **before** the bar is
  dispatched to signal generators (`universe_runner.py:331-334`).
- Boundary test uses `bar.ts.date()` where `Bar.ts` is **tz-aware UTC**
  (`src/strategies/sepa/signal.py:71-83`) — i.e. the UTC calendar date of
  the bar timestamp, not the ET date.
- First bar ever: record the date, do **not** fire (day 0 has no screener
  data) (`universe_runner.py:203-209`).
...
- `on_pre_open(target_date)`: ask screener `top_k(k=active_cap,
  ...
  symbol maps to a registered instrument; then enforce subscription cap
  (`universe_runner.py:220-239`). Defaults: `active_cap=20`,
  `subscription_cap=30` (`universe_runner.py:57-58`).

### 1.5 Market-session labels (quote stream)

Source: `src/data/quote_actor.py:164-175`.

**[MUST-MATCH]** moomoo `market_status` → session label mapping (uppercased
...
Source: `src/universe/history_provider.py:23-29`.

**[MUST-MATCH]** Interface (Go: a small interface):

- `get_bars(ticker, start, end) -> DataFrame` — date-indexed, columns at
  minimum `open, high, low, close, volume` (lower-case)
  (`history_provider.py:8-12`).
- `list_tickers(as_of) -> []string` — active universe symbols on `as_of`.

### 2.2 Ticker-window filtering (survivor-bias-free)

Source: `src/data/sharadar_cache/reader.py:23-97`.

**[MUST-MATCH]** `_filter_by_window(df, start, end)` keeps a TICKERS row iff:

- `firstpricedate` is missing/NaT **or** `firstpricedate <= end`
  (`reader.py:37-39`), **and**
- `lastpricedate` is missing/NaT/empty-string **or** `lastpricedate >= start`
  (`reader.py:40-43`; empty string coerces to NaT via
  `pd.to_datetime(..., errors="coerce")`, `reader.py:18-20`, and counts as
  "still active").
- If a date column is entirely absent, no filtering on it (keep all rows)
  (`reader.py:26-31`).
- Comparisons are date-inclusive on both ends.

**[MUST-MATCH]** `list_universe_for_window(start, end, table=...)`
(`reader.py:73-97`):

- Empty cache or missing `ticker` column → empty list (`reader.py:92-94`).
...
**[MUST-MATCH]** `list_active_tickers(as_of)` = same filter with
`start = end = as_of` (`reader.py:57-71`).

**[MUST-MATCH]** `get_bars(ticker, start, end)` (`reader.py:99-127`): scans
...
an exception (`reader.py:3-5`).

### 2.3 Market-cap lookup

Source: `scripts/multi_strategy_backtest.py:182-204` (used by both the
backtest assembly and `live_runner` via import at
`src/runner/live_runner.py:198-202,291,329`).

**[MUST-MATCH]** `_market_cap_lookup_factory(cache)` returns a memoized
`lookup(ticker) -> float`:

- Per-process memoization dict; first call per ticker hits
  `cache.get_fundamentals(ticker)` (`multi_strategy_backtest.py:188-194`).
- If SF1 frame is nil/empty or lacks a `marketcap` column → cache and return
  `0.0` (`multi_strategy_backtest.py:192-195`).
- Otherwise take the row with the **latest `datekey`**
  ...
  dimension filter (`reader.py:129-147`)
  (`multi_strategy_backtest.py:196-198`).
- `marketcap` of `None`/NaN → `0.0`; else `float(marketcap)`
  (`multi_strategy_backtest.py:199-200`). Units: USD (Sharadar SF1
  `marketcap` is USD).
- Semantics of `0.0`: "unknown ticker, treated as fail-rule-8 by the
  screener" (`multi_strategy_backtest.py:185`) and "sorts last" in the
  universe cap (§4).
...
Source: `src/universe/screener.py`.

**[MUST-MATCH]** Two-method contract (`screener.py:37-55`):
...
- `update(bar)` — per-bar rolling-state update; complexity contract is
  **O(1) per call** (fired ~3,500×/trading day; ~4.4M calls over a 5-year
  backtest, `screener.py:9-11`). No full recompute per bar.
- `top_k(k, as_of) -> []ScreenedCandidate` — once per trading day;
  O(N log N) acceptable (`screener.py:46-49`).

**[MUST-MATCH]** `ScreenedCandidate` value object (`screener.py:23-34`):
...
Source: `src/universe/sepa_screener.py:33-50`.

| Parameter | Default | Meaning | Source |
|---|---|---|---|
| `market_cap_lookup` | required | `func(ticker) -> USD market cap; 0.0 = unknown (fails rule 8)` | `sepa_screener.py:42-48` |
| `history_max_bars` | **260** | rolling per-ticker bar-tail cap (≈1 trading year; MA200 + 252-bar 52w window need ≥252) | `sepa_screener.py:30-33,49` |
| `market_cap_min_usd` | **500,000,000.0** | forwarded to trend-template rule 8 | `sepa_screener.py:50` |
| `_BREAKOUT_BASE_LOOKBACK` | **60** (constant) | breakout-proximity high/low window | `sepa_screener.py:34` |

### 3.3 Per-ticker rolling state and `update(bar)`

Source: `sepa_screener.py:53-104`.

**[MUST-MATCH]**:

- State per ticker: deque of `(ts, open, high, low, close, volume)` with
  floats for OHLC and int volume (`sepa_screener.py:57-59`); inputs are
  coerced `float(bar.open/high/low/close)`, `int(bar.volume)`
  (`sepa_screener.py:96-104`). `Bar` fields arrive as `Decimal` + tz-aware
  UTC `ts` (`src/strategies/sepa/signal.py:71-83`).
- On append: push right; pop left while `len > history_max_bars`
  (`sepa_screener.py:73-75`).
- After every append, recompute over the trailing
  `min(len, 60)` bars: `last_60_high = max(high)`, `last_60_low = min(low)`
  (note: window is whatever is available when fewer than 60 bars exist)
  (`sepa_screener.py:76-79`). Also store `last_close`, `last_ts`
  (`sepa_screener.py:80-81`).
- First bar for an unseen symbol creates state implicitly
  (`sepa_screener.py:91-95`).

**[MUST-MATCH]** `warmup(ticker, df)` (`sepa_screener.py:106-141`; tests
`tests/universe/test_sepa_screener.py:259-303`):
...
- Purpose: without warmup, every ticker has <200 bars on day 1 and the score
  ...
  (`sepa_screener.py:110-114`).

### 3.4 Breakout proximity

Source: `sepa_screener.py:150-164`; tests
`tests/universe/test_sepa_screener.py:63-119`.

**[MUST-MATCH]** `breakout_proximity(ticker)`:
...
- Unknown ticker → `0.0` (`sepa_screener.py:156-158`).
- Degenerate/flat range (`high_60 <= low_60`) → `0.0`
  (`sepa_screener.py:157-158`).
- Clamp below 0 → `0.0`; above 1 → `1.0` (`sepa_screener.py:160-164`).
  ...
  (test `test_sepa_screener.py:105-119`).

### 3.5 Trend Template count

Source: `sepa_screener.py:166-203`; `src/strategies/sepa/trend_template.py`.

**[MUST-MATCH]** `trend_template_count(ticker)`:

- Unknown ticker or zero bars → `0` (`sepa_screener.py:175-177`).
- Materialize deque → frame (`open/high/low/close/volume`, timestamp index)
  ...
  (count of true rules, 0–8) (`sepa_screener.py:178-184`,
  `trend_template.py:82-96`).
- Cost contract: only called from `top_k` (once per ticker per day), never
  from `update` (`sepa_screener.py:171-174`).

**[MUST-MATCH]** `evaluate` — the 8 Minervini rules
(`trend_template.py:99-174`):

Insufficient history, `n < 200` bars (`trend_template.py:113-134`):
...
(test `test_sepa_screener.py:134-142`).

With `n >= 200` (`trend_template.py:136-174`):
...
- MA = simple rolling mean of `close`, last value
  (`src/strategies/sepa/_indicators.py:13-15`, `trend_template.py:136-139`).
- 52-week window = **252 bars** on `high`/`low` columns respectively
  (`trend_template.py:38,141-142`; `_indicators.py:54-62`). Rolling
  ...
  (`trend_template.py:143-152`).
- Constants: `_HIGH_TOLERANCE = 0.25`, `_LOW_PREMIUM = 0.30`,
  `DEFAULT_MARKET_CAP_MIN_USD = 500_000_000.0`
  (`trend_template.py:37-40`).
- Diagnostic `ma200_uptrend_days` (`_indicators.py:34-52`): 0 when
  ...
  (`trend_template.py:17-19,173`).
- `passed` (all 8 true) exists but the screener uses only `passing_rules`.

**NaN handling note [MUST-MATCH]:** with `n >= 200` all three MAs are
...
Source: `sepa_screener.py:205-243`; tests
`tests/universe/test_sepa_screener.py:168-236`.

**[MUST-MATCH]**:

- `k <= 0` or no tracked tickers → empty slice (`sepa_screener.py:217-218`).
- For **every** tracked ticker: `tt = trend_template_count(t)`,
  `prox = breakout_proximity(t)`,
  `score = tt * 10.0 + prox * 5.0`,
  `cap = market_cap_lookup(t)` (`sepa_screener.py:221-226`).
- Sort key: `(-score, -market_cap, ticker)` — i.e. **score DESC, then
  market cap DESC (decision U-D7), then ticker ASC** as the final
  deterministic tiebreak (`sepa_screener.py:227-228`). Output is fully
...
- `as_of` is **informational only** — ranking uses whatever state has been
  ...
  close of `as_of` (`sepa_screener.py:210-215`).

**[IMPROVE]** Original: `market_cap_lookup` is called twice per ticker per
`top_k` (once inside `trend_template_count`, once for the sort key) and the
scored tuple stores the cap twice (`sepa_screener.py:219-226`). Memoization
...
Source: `src/runner/live_runner.py`.

### 4.1 Raw SEPA universe & exclusions

Source: `live_runner.py:256-301`.

**[MUST-MATCH]** Sequence (live mode, warmup enabled):

1. `today = date.today()` (process-local date);
   `warmup_start = today - 730 days` (`2 * 365`, no leap handling)
   (`live_runner.py:257-258`).
2. Raw SEPA universe =
   ...
   (`live_runner.py:278-281`).
3. **Exclusion set** `_non_stock = {"SPY"} ∪ sector_tickers`; the raw list is
   filtered to drop these, preserving order (`live_runner.py:282-284`).

**Excluded tickers — exact list [MUST-MATCH]:**

- `SPY` (regime/heartbeat instrument; `live_runner.py:283`).
- The 11 Select Sector SPDR ETFs, from
  `strategies.sector_rotation.DEFAULT_UNIVERSE`
  (`src/strategies/sector_rotation/signal.py:50-63`), in source order:
  `XLK, XLF, XLE, XLV, XLY, XLP, XLU, XLB, XLI, XLRE, XLC`.

**Pair legs — exact list [MUST-MATCH]:** from
`strategies.pairs.DEFAULT_PAIRS` (`src/strategies/pairs/signal.py:46-51`):
`("KO","PEP"), ("MA","V"), ("XOM","CVX")` → deduped + sorted leg set used for
subscriptions: `CVX, KO, MA, PEP, V, XOM` (`live_runner.py:260-261`).
**Pair legs are NOT excluded from the SEPA universe** — only SPY + the 11
sector ETFs are (`live_runner.py:283-284`). A pair leg that ranks in the SEPA
top-N appears in both lists; the per-ticker `bar_type_by` map dedups by key
(`live_runner.py:318-324`), but `build_all` receives the lists separately
(`live_runner.py:362`).

The exclusion happens **before** the market-cap cap is applied, so the limit
budget is spent entirely on stocks (`live_runner.py:282-294`).

**[MUST-MATCH]** Failure policy: each load step (SPY warmup, universe, SF1,
earnings) is independently try/except'd; any failure prints a WARN and
degrades (e.g. universe failure → empty SEPA universe), never aborts
(`live_runner.py:271-315`). The same applies to Sharadar auto-catchup
(`live_runner.py:229-254`; enabled by default, disabled by `--no-sync` or
env `TMS_AUTO_SYNC=0` — any other value, including unset, enables it,
`live_runner.py:233-234`).

### 4.2 `TMS_LIVE_UNIVERSE_LIMIT` resolution

Source: `live_runner.py:43-60`; tests
`tests/runner/test_live_universe_limit.py:79-98`.

**[MUST-MATCH]** `_resolve_universe_limit()`:
...
| non-integer | error: `TMS_LIVE_UNIVERSE_LIMIT must be an integer, got {raw!r}` — **fail fast at startup**, no silent fallback (`live_runner.py:55-60`, test `:93-98`) |

Rationale for 85 (`live_runner.py:43-47`): moomoo OpenD caps one account at
...
Source: `live_runner.py:63-87`; tests
`tests/runner/test_live_universe_limit.py:26-76`.

**[MUST-MATCH]** `_apply_universe_limit(sepa_tickers, market_cap_lookup,
limit)`:

1. `limit <= 0` **or** empty input → empty tuple (`live_runner.py:82-83`;
   tests `:26-36`). Negative limits are valid input (defensive) and yield
   empty.
2. `len(input) <= limit` → return input **unchanged, original order
   preserved** — no reshuffling (`live_runner.py:84-85`; tests `:39-49`).
3. Otherwise: sort by `market_cap_lookup(t)` **descending** and take the
   first `limit` (`live_runner.py:86-87`).

**Ordering details [MUST-MATCH]:**

- The sort is Python's `sorted(..., reverse=True)` — a **stable** sort.
...
(`live_runner.py:317-324`).

**[IMPROVE]** Original: the cap is a blunt market-cap ranking ("ignore the
rest until live universe is refined offline — V3-D follow-up",
`live_runner.py:73-74`); the screener's own quality score is ignored at
...
**[MUST-MATCH]** The capped `sepa_tickers` drives (a) SF1 MRT bulk load —
pyarrow dataset filtered to `ticker ∈ list` AND `dimension == "MRT"`, columns
`ticker, datekey, dimension, marketcap`
(`scripts/multi_strategy_backtest.py:223-244`, called at
`live_runner.py:303-308`); (b) earnings load — EVENTS rows whose pipe-split
`eventcodes` contains the literal code `"22"`, with column `date` renamed to
`report_date` (`multi_strategy_backtest.py:246-266`, called at
`live_runner.py:310-315`); (c) the screener instance built with the same
memoized market-cap lookup (`live_runner.py:328-330`).

**[MUST-MATCH]** Backtest parity: the backtest assembly uses the identical
exclusion rule `_non_stock = {"SPY", *DEFAULT_UNIVERSE}` on the
`table="SF1"` window universe (`multi_strategy_backtest.py:395-397,527-531`),
but applies **no** top-N cap (no subscription quota in backtest). The Go
backtest path must likewise not cap.

### 4.5 Runner-side caps that interact with the universe

Source: `universe_runner.py:240-303` (context for completeness; the runner
itself is another spec's scope).

- `active_cap = 20` (U-D2), `subscription_cap = 30` (U-D3)
  (`universe_runner.py:57-58`).
- **[MUST-MATCH]** Cap enforcement (U-D12): while
  ...
  is evictable, log a warning and stop (`universe_runner.py:258-303`).
...
|---|---|---|---|
| `TMS_LIVE_UNIVERSE_LIMIT` | 85 | tickers | `live_runner.py:47,50-54` |
| moomoo subscription hard cap (external) | 100 | K-line subs/account | `live_runner.py:43-46` |
| Warmup window | 730 (`2*365`) | calendar days | `live_runner.py:258` |
| Universe table filter | `"SF1"` | — | `live_runner.py:279-280` |
| Excluded: heartbeat | `SPY` | — | `live_runner.py:283` |
| Excluded: sector ETFs (11) | `XLK XLF XLE XLV XLY XLP XLU XLB XLI XLRE XLC` | — | `sector_rotation/signal.py:50-63` |
| Pair legs (subscribed, NOT excluded) | `CVX KO MA PEP V XOM` | — | `pairs/signal.py:46-51`; `live_runner.py:260-261` |
| `history_max_bars` | 260 | bars | `sepa_screener.py:33,49` |
| Breakout lookback | 60 | bars | `sepa_screener.py:34` |
| 52w high/low window | 252 | bars | `trend_template.py:38` |
| High tolerance (rule 6) | 0.25 | fraction | `trend_template.py:39` |
| Low premium (rule 7) | 0.30 | fraction | `trend_template.py:40` |
| Market-cap min (rule 8) | 500,000,000.0 | USD | `trend_template.py:37` |
| Score weights | TT×10.0 + proximity×5.0 | — | `sepa_screener.py:224` |
| `active_cap` / `subscription_cap` | 20 / 30 | tickers | `universe_runner.py:57-58` |
| RTH window | 09:30–16:00 inclusive | ET minutes-of-day 570–960 | `daily_aggregator.py:52-53` |
| Aggregation timezone | `America/New_York` | IANA | `daily_aggregator.py:51` |
| Catch-up target | T-1 | UTC date | `catchup.py:114-115` |
| `TMS_AUTO_SYNC` | `"1"` (on; `"0"` disables) | — | `live_runner.py:233-234` |
...
1. **Sector-ETF count discrepancy in the quota comment.** The 85 default is
   justified as "SPY + sector ETFs (10) + pair legs (~10) take ~15"
   (`live_runner.py:43-46`), but `DEFAULT_UNIVERSE` has **11** ETFs and there
...
2. **Market-cap lookup ignores SF1 dimension.** `_market_cap_lookup_factory`
   takes the latest row by `datekey` across **all** dimensions
   (`multi_strategy_backtest.py:188-198`), while `_load_sf1_mrt` filters to
   `MRT` only (`multi_strategy_backtest.py:223-244`). If multiple dimensions
...
3. **Pair legs intentionally not excluded from SEPA?** `KO/PEP/MA/V/XOM/CVX`
   ...
   trade names that the pairs strategy is simultaneously long/short
   (`live_runner.py:282-294`). Confirm this overlap is accepted (it also
   slightly relieves the subscription quota via dedup).
4. **Catch-up start day re-fetch.** `start = last_sep.date()` re-fetches the
   already-synced day every startup (`catchup.py:116`). Idempotent but wastes
   two API calls; should Go start at `last_sep.date() + 1`? (Behavior change —
   default should match reference.)
5. **Pre-open boundary uses the UTC date of bar timestamps**
   (`universe_runner.py:206`, `signal.py:74`). For 1-DAY bars whose
   ...
   may keep UTC-date semantics (required for parity) and document the
   assumption that daily bars are stamped at RTH close.
6. **`warmup_start` uses calendar days, not trading days** (`2*365`,
   `live_runner.py:258`) — yields ~500 trading days, comfortably above the
...
7. **Half-day sessions** (e.g. day after Thanksgiving, Christmas Eve): the
   ...
   until 16:00; the daily bar is only emitted on the next day's first RTH bar
   (`daily_aggregator.py:34-39`). Acceptable for Go v1, or implement the
   [IMPROVE] early-close table from §1.1?
8. **`date.today()` in live universe assembly** (`live_runner.py:257`) is the
