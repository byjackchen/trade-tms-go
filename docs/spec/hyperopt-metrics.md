...
A Go engineer with no access to the Python repo must be able to implement a
byte-equivalent system from this document alone.

Source-of-truth files (all paths relative to the Python repo root):

| Area | File |
|---|---|
| Metrics | `src/research/metrics.py` (77 lines) |
| Walk-forward splitter | `src/research/walkforward.py` (71 lines) |
| Search-space registry | `src/research/search_spaces.py` (59 lines) |
| Params loader / search JSON | `src/strategies/params/loader.py` (272 lines) |
| Constraint expression evaluator | `src/strategies/params/safe_eval.py` (68 lines) |
| Baseline search-space JSONs | `src/strategies/params/baseline/{sepa,sector_rotation,pairs}.json` |
| Optimizer / coordinator | `src/research/optimizer.py` (636 lines) |
| Trial worker | `src/research/workers.py` (218 lines) |
| Artifacts | `src/research/artifacts.py` (79 lines) |
| Export helpers (top/Pareto) | `src/research/export.py` (65 lines) |
| CLI | `scripts/run_hyperopt.py` (75 lines) |
| Backtest entry (metrics source) | `scripts/multi_strategy_backtest.py` (835 lines; only the parts hyperopt depends on are specced here) |
| API readers (study list/detail, staleness) | `src/api/hyperopt_reader.py` (120 lines) |
| Promotion (`runs/active_params/`) | `src/api/strategy_source.py` (193 lines), `src/api/deps.py:126-133` |
| Design docs | `docs/PLAN-HYPEROPT.md`, `docs/adr/ADR-006-hyperopt-architecture.md` |
| Tests encoding semantics | `tests/research/*.py` (15 files, 1342 lines) |

Tagging convention used throughout:

- **[MUST-MATCH]** — Go must replicate exactly (formulas, edge cases, NaN/zero
  handling, rounding, ordering, file names, JSON shapes).
- **[IMPROVE]** — known weakness; Go may do better. Both the original behavior
  and the proposed improvement are described. If the Go implementation chooses
  not to improve, it must replicate the original.
...
- All wall-clock timestamps are **UTC**, RFC 3339 / ISO-8601 with offset
  ...
  microsecond precision, `+00:00` suffix). `optimizer.py:55-56`. [MUST-MATCH]
- Study directory timestamps use UTC format `%Y-%m-%d_%H-%M-%S`
  (e.g. `2026-05-04_17-06-29`). `optimizer.py:59-60`. [MUST-MATCH]
- Calendar dates (`start`, `end`, fold boundaries) are timezone-less ISO dates
  `YYYY-MM-DD` treated as calendar days, not trading days. [MUST-MATCH]
- Money unit is USD throughout; percentages are expressed in percent units
  (e.g. `max_drawdown_pct = -10.0` means −10%).

---

## 1. Performance metrics (`src/research/metrics.py`)

Pure functions over an equity curve `[]float64`. No NumPy — Python uses
`statistics.mean`, `statistics.pstdev`, `math.sqrt` on `float` (IEEE-754
double). Go `float64` arithmetic matches.

### 1.1 `BacktestMetrics` struct (`metrics.py:9-25`)

[MUST-MATCH] Exact field set and JSON key names (this dict is serialized
...
[MUST-MATCH] `to_objectives()` returns the tuple `(sharpe, calmar)` — this is
the objective ordering reported to the optimizer (`metrics.py:24-25`,
verified by `tests/research/test_metrics.py:29-42`).

### 1.2 Simple returns helper (`metrics.py:28-34`)

[MUST-MATCH] Per-period simple returns over consecutive pairs:
...
### 1.3 Sharpe (`metrics.py:37-44`)

[MUST-MATCH] Formula, defaults, and edge cases:
...
- Annualization factor: `sqrt(periods_per_year)`, default `periods_per_year = 252`.
  No call site overrides the default. [MUST-MATCH]
- Risk-free rate: **none** (raw mean return). [MUST-MATCH]
- `pstdev` is population standard deviation (divide by N, not N−1). In Go:
  `sqrt(sum((x-mean)^2)/N)`. [MUST-MATCH]
- Flat curve → 0.0 (test `test_metrics.py:17-18`). [MUST-MATCH]
- Fewer than 2 returns (curve shorter than 3 points after zero-skip) → 0.0.
  [MUST-MATCH]

[IMPROVE] `statistics.mean`/`pstdev` in CPython use exact-fraction summation
internally, then convert to float; a naive Go running sum can differ in the
last ulp. Original: exact rational mean. Improvement/parity option: use
Kahan/Neumaier compensated summation (or math/big rational for the mean) so
results agree to full float64 precision with the Python reference. The Go
implementation must at minimum match the Python output to within 1e-12
relative for the reference vectors (see §13 verification).

### 1.4 Max drawdown percent (`metrics.py:47-59`)
...
- Returns a **non-positive percent** (e.g. `[100k,110k,99k,120k]` → −10.0;
  test `test_metrics.py:13-14`). [MUST-MATCH]
- Empty curve → 0.0; monotonic-up curve → 0.0 exactly. [MUST-MATCH]
- A zero peak is skipped (no division by zero). [MUST-MATCH]

### 1.5 Calmar (`metrics.py:62-77`)

[MUST-MATCH] Exact algorithm including all special cases:
...
- The period count is `len(curve) - 1` samples at `periods_per_year = 252`
  (curve is EOD-sampled). [MUST-MATCH]
- Zero-drawdown positive-growth curve → `ann / 0.01` (synthetic 1% DD floor),
  test `test_metrics.py:25-26`. [MUST-MATCH]
- Total wipeout (`total_return ≤ −1`) → exactly −1.0. [MUST-MATCH]
- Annualization is geometric: `(1+tr)^(1/years) − 1`, with `years` floored at
  one period (`1/252`). [MUST-MATCH]

### 1.6 Where the equity curve comes from (`scripts/multi_strategy_backtest.py`)

(Scope note: the backtest engine itself is covered by the backtest spec; the
following is the contract hyperopt depends on.)

- `run_backtest` signature relevant to hyperopt (`multi_strategy_backtest.py:361-372`):
  ...
  pre_open_count, equity_curve []float64}` (`:171-179`). [MUST-MATCH]
...
## 2. Search-space JSON format (`src/strategies/params/loader.py` + baseline JSONs)

### 2.1 File schema

One JSON file per strategy at `<params_dir>/<strategy>.json`. [MUST-MATCH]
Top-level shape:
...
Validation rules (`loader.py:129-185`), all errors are `ValueError` with the
messages shown: [MUST-MATCH]
...
order (§2.3) and output file order (§8). [MUST-MATCH]

### 2.2 File resolution order (`loader.py:64-126`)

`load_strategy_params(strategy, params_dir=None)`: [MUST-MATCH]
...
2. Else if env `TMS_STRATEGY_PARAMS_DIR` is set (via app config,
   `src/config.py:59,66,75` — unset/empty → nil) **and**
   `<env_dir>/<strategy>.json` exists: use it.
3. Else: package baseline dir `src/strategies/params/baseline/<strategy>.json`;
...
(`loader.py:72-77`). [MUST-MATCH]

Any error loading config in step 2 silently degrades to baseline
(`loader.py:99-110`). [IMPROVE] Original: bare `except Exception: return None`
...
### 2.3 Suggestion semantics (`loader.py:192-230`)

`suggest_with(params, trial)`: [MUST-MATCH]
...
- Returns ONLY sampled keys; static defaults are NOT merged in
  (`loader.py:196-203`; verified `test_search_spaces.py:32-43`: sepa yields
...
- After sampling, apply constraints **in file order**, each one:
  ...
  value for that parameter remains the raw suggested value. [MUST-MATCH]
...
(`loader.py:187-189`). [MUST-MATCH]

### 2.4 Constraint expression language (`safe_eval.py`)

[MUST-MATCH] AST-whitelisted expression evaluator:
...
grammar; numeric results are float64 (Python int literals may produce ints —
...
(`baseline/pairs.json:54-60`; test `test_search_spaces.py:53-56`). [MUST-MATCH]

### 2.5 Baseline search spaces (verbatim data) [MUST-MATCH]
...
### 2.6 Registry (`src/research/search_spaces.py`)

[MUST-MATCH]

- `SEARCH_SPACES` keys exactly `{"sepa", "sector_rotation", "pairs"}`
  (test `test_search_spaces.py:21-22`).
...
## 3. Walk-forward split algorithm (`src/research/walkforward.py`)
...
### 3.1 `expanding_anchored(start, end, n_folds=5, embargo_days=5)` [MUST-MATCH]
...
Properties guaranteed (encoded in `tests/research/test_walkforward.py`):

- Segments are strictly ordered, non-overlapping, fully inside `(start, end]`
  with `test_start > start`. [MUST-MATCH]
- The final fold ends exactly at `end`. [MUST-MATCH]
- **Embargo quirk** [MUST-MATCH]: because the embargo offset is constant
  ...
  docstring claims otherwise; the test comment (`test_walkforward.py:26-28`)
...
[IMPROVE] Original is calendar-day based and the first-third buffer is a
...
### 3.2 Fold construction in the coordinator (`optimizer.py:215-230, 504-519`)

[MUST-MATCH] Folds are computed **once** in the coordinator from
...
## 4. Per-fold metric aggregation (`src/research/workers.py:83-146`)

### 4.1 Equity-curve stitching (`workers.py:83-109`) [MUST-MATCH]
...
### 4.2 Recompute, don't average (`workers.py:112-146`) [MUST-MATCH]
...
Rationale (docstring `workers.py:117-121`): the previous design mixed
...
`test_workers.py:108-151` (two positive folds ⇒ both > 0, final ≈
starting·1.02·1.03, maxDD == 0.0) and `:154-189` (two losing folds ⇒ sharpe,
calmar, maxDD all < 0). [MUST-MATCH]

### 4.3 Per-fold payloads [MUST-MATCH]
...
key first (`workers.py:168`). These go into `trial_*.json` `folds` array in
fold order. In single-window mode `folds = []`.

---

## 5. Trial worker (`src/research/workers.py:149-218`)

### 5.1 `WorkerConfig` [MUST-MATCH]
...
processes (Python pickles it through spawn).

### 5.2 Override mapping (`workers.py:63-80`) [MUST-MATCH]
...
### 5.3 Execution paths (`workers.py:149-196`) [MUST-MATCH]
...
- **Single window** (`folds` nil/empty): one `run_backtest` over
  ...
  nil.
- `bypass_logging=True` always (Nautilus/Rust logging can only init once per
  process — ADR-006). [MUST-MATCH]

### 5.4 Result envelope [MUST-MATCH]
...
- Timeout: same as FAIL but `error = "timeout: trial timeout after <N>s"`
  ...
  exception text `trial timeout after <N>s`; `workers.py:42,203`).
- `duration_sec` = monotonic elapsed seconds, measured from worker entry to
  return, including failures (`>= 0`; test `test_workers.py:243`).

### 5.5 Per-trial timeout (`workers.py:31-50`)
...
(test `test_workers_timeout.py:10-39`: 1s timeout fires well before a 5s
sleep completes; `duration_sec >= 0.9`).

[IMPROVE] Original weaknesses: SIGALRM only works in the process main thread;
...
## 6. Optimizer / study coordinator (`src/research/optimizer.py`)

### 6.1 `run_study` parameters and validation [MUST-MATCH]
...
### 6.3 Resume mismatch guard (`optimizer.py:432-454`) [MUST-MATCH]
...
### 6.4 Optuna study configuration (`optimizer.py:243-257`) 

- Multi-objective, `directions = ["maximize", "maximize"]` over objectives
  `("sharpe", "calmar")` in that order. [MUST-MATCH]
- Sampler: **NSGA-II** seeded with `seed` (`NSGAIISampler(seed=seed)`),
  Optuna 4.8.0 defaults (verified in installed package
  `optuna/samplers/nsgaii/_sampler.py:156-176`): [MUST-MATCH at the
  algorithm/semantics level — see Open Question Q1 for numeric stream parity]
  ...
  - Child generation (`_child_generation_strategy.py:58-103`): with
    probability `crossover_prob = 0.9` perform **uniform crossover** of two
    parents (per-parameter swap with `swapping_prob = 0.5`,
    `_crossovers/_uniform.py:37-49`; retried until the child lies inside the
    ...
    (binary tournament on Pareto dominance only — no crowding tiebreak;
    `_crossover.py:140-155`); the second parent is drawn from the population
    excluding the first.
...
- Storage: Optuna `JournalStorage` over append-only file
  ...
  sampler history. [MUST-MATCH in function: the Go implementation must
  persist optimizer state sufficient to resume the NSGA-II population from
  disk; see Q2 for format.]
- Objective reporting (`optimizer.py:288-292`): COMPLETE →
  `tell(trial, (float(metrics["sharpe"]), float(metrics["calmar"])))`; any
  other state → `tell(trial, state=FAIL)`. [MUST-MATCH]

### 6.5 Trial numbering and resume skip (`optimizer.py:73-101, 484-485`) [MUST-MATCH]
...
- The artifact number is injected into the suggested params map as
  ...
  before writing the artifact and before building overrides).
  [MUST-MATCH — worker stubs in tests read it.]
- The optimizer-internal trial number (Optuna's `trial.number`, which grows
  ...
  with no failures they coincide. [MUST-MATCH]

### 6.6 Sequential mode (`workers == 1`; `optimizer.py:295-319, 520-536`) [MUST-MATCH]
...
### 6.7 Parallel mode — ProcessPool layout (`optimizer.py:41-43, 322-399`) [MUST-MATCH]
...
- **Streaming submit with in-flight cap** (test
  `test_optimizer_streaming.py`): submit at most `workers` tasks up front;
...
- Completion loop: wait FIRST_COMPLETED over in-flight futures; for each done
  ...
  error:str(exc), duration_sec:0.0}` [MUST-MATCH the 0.0]; `finished_at=now`;
...
- [IMPROVE] `last_error` is overwritten on EVERY completion — a success
  ...
  reference semantic.
- [IMPROVE] Synthesized FAIL on future exception records
...
### 6.8 Progress writes (`optimizer.py:126-156`) [MUST-MATCH]
...
- `current_best` = over all COMPLETE trial artifacts with numeric sharpe and
  ...
  null when none. [MUST-MATCH — the API and source-picker labels read this.]
  [IMPROVE] The sum is an arbitrary scalarization of a Pareto pair; Go may
...
- `coordinator_pid` = PID captured at module import (the process that started
  the study) [MUST-MATCH field; value is process-specific].
- [IMPROVE] O(trials²) file re-reads over a study; Go may cache counters in
  memory (output must remain identical).

Write points: (1) RUNNING before any trial dispatch — deliberately before
data prefetch so an early interrupt still leaves a flippable progress file
(`optimizer.py:488-496`); (2) per-trial/per-completion as above;
(3) terminal states (§6.9).

### 6.9 Lifecycle states and interruption (`optimizer.py:504-578`) [MUST-MATCH]
...
- Any other exception: write `status="INTERRUPTED"` with
  `last_error=str(exc)`, re-raise (test `test_optimizer_interrupt.py`).
...
### 6.10 Heartbeat (`optimizer.py:159-212`) [MUST-MATCH]
...
- Each tick: read `progress.json`; if missing or unparseable or the write
  fails → silently no-op (file preserved verbatim on corrupt JSON — test
  `test_optimizer_heartbeat.py:66-71`); else set ONLY `last_heartbeat_at` and
...
## 7. Artifact files (`src/research/artifacts.py`)

### 7.1 Atomic write [MUST-MATCH]
...
trailing newline. [IMPROVE] No fsync before rename — a power loss can leave
an empty file; Go should fsync file and directory, content unchanged.

### 7.2 `study.json` — exact schema (field order = write order) [MUST-MATCH]
...
### 7.3 `progress.json` — exact schema [MUST-MATCH]
...
### 7.4 `trials/trial_%04d.json` — exact schema [MUST-MATCH]
...
### 7.5 Directory layout (ADR-006) [MUST-MATCH]
...
### 8.1 `best_params/` writing (`optimizer.py:592-636`) [MUST-MATCH]
...
1. `candidates = study.best_trials` — the **Pareto-optimal COMPLETE trials**
   ...
   `test_optimizer_writes_best_params.py:79-103`).
2. `best_trial = argmax over candidates of values[0]` (highest **sharpe**;
   missing values treated as −inf; Python `max` keeps the FIRST maximal
...
### 8.2 `write_tuned_params` (`loader.py:233-272`) [MUST-MATCH]

- Read baseline `base_dir/<strat>.json` (the **package baseline**, not the
  env dir — `optimizer.py:618`); missing → FileNotFoundError.
...
### 8.3 Promotion path A — env var (CLI) [MUST-MATCH]

`scripts/run_hyperopt.py --promote-best` only **prints** (never edits files):
...
### 8.4 Promotion path B — `runs/active_params/` (API SourceManager) [MUST-MATCH]
...
directory (`api/deps.py:126-133`: `active_dir = TMS_RUNS_DIR(default
"runs")/active_params`, hyperopt dir `runs/hyperopt`).

`src/api/strategy_source.py`:
...
- API endpoints (`api/server.py:1108-1148`):
...
- Effect is **next-run-only**: live processes read params at startup; nothing
  hot-reloads (design intent, header comment `strategy_source.py:1-11`).

---

## 9. Read-side API contract

### 9.1 Endpoints (`api/server.py:495-520`) [MUST-MATCH]
...
### 9.2 Staleness override (`api/hyperopt_reader.py:9-38`) [MUST-MATCH]
...
[IMPROVE] PID liveness is meaningless across container namespaces (false
...
## 10. Export helpers (`src/research/export.py`)

Operate on trial-artifact maps (as read from disk).

- `_metric(trial, name)`: `trial.metrics[name]` if int/float (bool is a
  Python int subtype — a `true` would coerce to 1.0; irrelevant in practice)
  else nil. [MUST-MATCH]
- Only `state == "COMPLETE"` trials considered, both helpers. [MUST-MATCH]
- `top_trials(trials, metric, limit=10)`: filter metric-present trials, sort
  ...
  `limit`. [MUST-MATCH]
  [IMPROVE] Sort key is `value or -inf`: a metric equal to **0.0** is falsy
  in Python and sorts as −inf (below any negative value). Original behavior
...
- `pareto_trials(trials, x_metric="sharpe", y_metric="calmar")`: candidate
  ...
  applies to comparison values of exactly-0.0 metrics ([IMPROVE], same
  treatment as above). [MUST-MATCH otherwise]

---

## 11. CLI and Make integration (`scripts/run_hyperopt.py`, `Makefile:56-57`)

[MUST-MATCH] (Go: same flags on the equivalent cobra/flag command)
...
| `--trial-timeout-sec` | int | 600 | **0 → disabled (nil)** (`run_hyperopt.py:58`) |
...
|---|---|---|
| Objectives / directions | (sharpe, calmar) / (max, max) | optimizer.py:253,290,468-469 |
| Sampler | NSGA-II, seed=42 default | optimizer.py:250; CLI default |
...
| periods_per_year | 252 | metrics.py:37,62 |
| Calmar zero-DD divisor | 0.01 | metrics.py:76 |
| Walk-forward defaults | 5 folds, 5 embargo days | optimizer.py:412-413 |
| Buffer | total_days // 3, min 1 | walkforward.py:54 |
| Trial timeout default | 600 s | optimizer.py:416; run_hyperopt.py:27 |
| Heartbeat interval | 20 s | optimizer.py:201 |
| Stale heartbeat threshold | 60 s | hyperopt_reader.py:9 |
| Starting balance | 100,000 USD | multi_strategy_backtest.py:370 |
| Strategy warmup | 400 calendar days (SPY: 500) | multi_strategy_backtest.py:409,346 |
| Study ts format | UTC `%Y-%m-%d_%H-%M-%S` | optimizer.py:59-60 |
| Trial file pattern | `trial_%04d.json` | artifacts.py:77 |
| current_best score | sharpe + calmar, strict-> first wins | optimizer.py:104-123 |
| Runs dir default | `runs/hyperopt`; active dir `runs/active_params` | optimizer.py:408; deps.py:128-132 |

---

## 13. Verification plan for the Go port

1. **Metric golden vectors**: run the Python venv
   (`.venv/bin/python -c "from research.metrics import *; ..."`) over a fixed
   ...
   IMPROVE).
2. **Walk-forward**: enumerate (start, end, folds, embargo) grids in Python
...
5. **Suggest/constraints**: drive `suggest_with` with a deterministic fake
   trial (mid-point sampling like `test_search_spaces.py:8-18`) in both
   languages; diff param maps including clamped pairs `exit_z`.
6. **Optimizer**: stub the worker fn (as the Python tests do) and compare the
...
1. **Q1 — NSGA-II numeric parity.** Optuna's NSGA-II consumes a NumPy
   MT19937 `RandomState` seeded with `seed`; byte-equivalent suggestion
   streams in Go would require porting NumPy's RandomState consumption
   pattern (rand/choice call ordering, including the retry loop in
   `perform_crossover` and the search-space transform). Is **semantic**
   equivalence (same algorithm, same hyper-defaults, deterministic per Go
   seed, self-consistent resume) acceptable, with cross-language parity
   asserted only at the metrics/artifact-schema level? If strict parity is
   required, the spec needs an addendum porting
   `optuna/samplers/nsgaii/*` + `_SearchSpaceTransform` + MT19937 call
   ordering. (Recommended: semantic equivalence; tag the sampler stream
   itself IMPROVE.)
2. **Q2 — Journal file compatibility.** Must the Go optimizer write an
   Optuna-compatible `optuna_journal.log` (JSON-lines op log readable by
   Python tooling), or is any durable state file at the same path
   semantics (resume restores asked/told trials and population) acceptable?
   Nothing else in the repo reads the journal file.
3. **Q3 — Optuna ask/tell trial-number drift on resume.** After a crash,
   the journal may contain asked-but-never-told trials (left RUNNING/stale),
   so Optuna trial numbers drift further from artifact numbers and
   `tuned_from_trial` (§8.1) refers to an Optuna number that has no
   `trial_NNNN.json` counterpart. Replicate this drift, or define Go trial
   identity as artifact-number-only (cleaner; affects only the provenance
   integer)? Original behavior: drift exists.
...
7. **Q7 — Trial artifact `params` for joint studies.** Joint trials store
   ...
   Confirm no consumer requires flat prefixed keys for joint studies (none
   found in the Python repo).
...
9. **Q9 — Stale `prefetch` module.** `src/research/__pycache__/` contains a
   compiled `prefetch` module with no corresponding source; ADR-006's "See
   also" references a Sharadar-prefetch plan. The current source has no
   prefetch step (each worker hits the parquet cache directly). The Go port
   should not implement a prefetch step unless the data-layer spec requires
   one.
