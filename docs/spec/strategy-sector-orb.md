...
> 本文档是本库对 sector_rotation 与 intraday_breakout 两套策略的定义。下述规则是
> 本系统的不变式,含所有边界情况、舍入、排序、字符串格式。凡指出"已知弱点"之处,
> 同时给出本库采用的更优行为。
...
- SG 层零交易框架依赖,接口为 `on_bar(Bar) -> list[Signal]`,
 外加 `evaluate_intent(as_of)`、`state_summary`、`state_dict`、`load_state(d)`。
 策略包不得依赖执行引擎包(纯逻辑层零交易框架依赖)。
...
- `on_bar` 模板 顺序:
 ...
 发布步骤的任何异常只记日志、不得影响交易路径。
- 组合闸门 `_gate`:若注入了 Portfolio 服务,先以
 `ProposedOrder(strategy_id=str(self.id), symbol, side, qty=target_qty, price=_last_close.get(symbol, 0), ts)`
 询问风控;拒绝则不下单。(语义:闸门在 Signal→Order 翻译之前)

---

## 1. 参数文件与超参搜索(两策略共用机制)

参数基线为 JSON 文件 `internal/hyperopt/baseline/<strategy>.json`,由
 加载。
...
- 调参产物 `write_tuned_params`:只覆写 `default` 与 `metadata`
 (`source="tuned"`、`tuned_from_study/trial`),search 范围与 constraints 原样保留,
 输出 `json.dumps(indent=2, sort_keys=False)`。
- **注意**: 的 `StrategyName = Literal["sepa","sector_rotation","pairs"]`
 —— `intraday_breakout` **未注册**进 Optuna 搜索空间(其 JSON 里写了 search 范围但当前优化器不会用)。
 (完整复刻:保留 ORB 的 search 元数据,但默认优化器不遍历 ORB)

---

## 2. 策略 A:sector_rotation(行业轮动)

模块构成:signal generator(核心)、intent(UI 快照类型)、runner(引擎封装),
基线参数 `internal/hyperopt/baseline/sector_rotation.json`。

### 2.1 概要

11 只 Select Sector SPDR ETF 的宇宙;每个自然月第一根 bar 触发再平衡;按 N 根 bar
回报排名,等权持有 top-K;只对"变更项"发信号(退出发 FLAT、新进发 LONG、留任不动)。
日线级别(生产环境 bar 类型为 `*-1-DAY-LAST-EXTERNAL`; 、)。

### 2.2 默认宇宙 (声明顺序即 tie-break 顺序; 见 2.6)
...
SG 配置 `SectorRotationSignalGeneratorConfig`(frozen):

| 参数 | 类型 | 默认(baseline JSON) | hyperopt 搜索范围 | 校验 | 来源 |
|---|---|---|---|---|---|
| equity_provider | 无参回调 → Decimal | **必填,无默认** | — | 必须 callable,否则 `TypeError`("equity_provider")。构造时**不得调用**它 |
...
| timezone | str | **"America/New_York"** | 不可搜索 | 无校验 | sector_rotation.json |
 ...
 上述默认、范围、校验消息关键字(由测试以消息匹配锚定)。

 `timezone` 字段在 sector_rotation 的**任何逻辑里都未被使用**
——月份翻转用的是 `bar.ts.date` 即 **UTC 日期**,不做时区换算。
...
> **单策略闸门(FIXER round 2 finding 1)**:SectorRotation 每个 pick
> 取 `equity/top_k`(基线 top_k=3 即 33% NAV)。多策略路径下跑 sector,33% > 30%
> 预算被 allocator 全部拒单,sector 在该路径成交为零——多策略路径**保留**此行为
> 以维持 hyperopt 目标面一致。单策略路径(live 默认 `--strategy sector_rotation`)
> 则不受此约束。该单策略
...
on_bar(bar):
 ...
 # 只比较"月份数字"!同月不同年(隔整 12 个月的数据缺口)不会触发。见 注记。
 4. signals = []
...
- **再平衡先于摄入新月 bar **:再平衡用的是"上月末的快照"——所有符号
...
3. ranked = 按 returns 值降序排序;排序必须**稳定**,并列时保持插入顺序
 = 宇宙声明顺序(稳定排序)。new_topk = ranked 前 top_k 的符号集合。
...
要点:
- **信号顺序 **:所有 FLAT 在所有 LONG 之前;组内按 symbol 字典序。
- **每次再平衡都调用 equity_provider **(PA-D1 不变量):
 4 倍权益 → 股数约 4 倍(`//` 截断容差内)。
- 留任持仓不按新权益重新定尺寸 → 权重随时间漂移,"等权"只对新进腿成立。
 原行为:留任完全不动;改进方案:可选的全量目标重定(emit 调整单)。
- `old <= 0` 时该符号被静默剔除出排名(与 evaluate_intent 中 `old <= 0 → return 0.0` 的处理**不一致**; vs 276-279)。
 Go 默认各自照抄两种行为。

### 2.7 evaluate_intent(as_of)

返回 `list[SectorRotationIntent]`,**每个宇宙符号恰一条,顺序 = 宇宙声明顺序**。
每次调用先 `_intent_generation += 1`(所有条目共享同一 generation; 严格单调)。

意图字段:`symbol, state, strength(0..100), proximity_to_trigger_pct
...
状态枚举:`no_setup / forming / buy / hold / exit / stop_hit`
(stop_hit 本策略不用)。

逻辑:
1. 暖机未完成(任一符号短缺)→ **全部** NO_SETUP、strength=0、proximity=null、
 rank=0、target_weight=0、current_weight=0(防 UI 闪烁)。
...
- `equity_at_snapshot` 必须存在且 `account_size` 键**必须不存在**。
...
模块构成:signal generator(核心)、intent(UI 快照类型)、runner(引擎封装),
基线参数 `internal/hyperopt/baseline/intraday_breakout.json`。
...
会话开盘时间**硬编码 09:30 交易所本地时间**:

```
_SESSION_OPEN_HOUR = 9; _SESSION_OPEN_MINUTE = 30
```

(注释明示:ORB 是美股形态,别的市场应另立策略模板)。

### 3.2 配置参数表 (intraday_breakout.json)
...
- `_lock_range`:
 ...
 (:区间末 bar 处理完后 locked 仍为 false)。
- 任何 `local_ts < range_end` 的 bar 都计入区间——包括 09:30 之前的
 盘前 bar(如 09:00 的 bar 也满足 `< 10:00`)。原行为:盘前 bar 会污染开盘区间;
 改进方案:加 `local_ts >= session_open` 下界过滤。Go 默认照抄原行为
 (生产数据流仅推送盘中 bar 时两者等价)。
- `local_ts.replace(hour=9,...)` 在 DST 切换日按本地挂钟时间定位 09:30,
 时区语义下结果正确;但若 bar 时间戳本身落在不存在/重复的本地时刻,需按 fold
 语义处理。实现用 `time.Date(y,m,d,9,30,0,0,loc)` 等价构造即可;测试仅覆盖
 冬令时日期。

### 3.5 入场 `_maybe_enter(bar, local_ts)`

```
1. local_ts >= eod_dt → 返回 [] # EOD 当口及之后禁止开新仓
 eod_dt = local_ts.replace(hour=HH, minute=MM, second=0, microsecond=0) # 解析 eod_exit_time
2. range_high 或 range_low 为 null → []
3. 突破条件(都是严格不等):
 bar.close > range_high # 等于不算
...
数值锚点:equity=100000、risk_pct=1.0、hard_stop_pct=1.0、
...
hard_stop_pct=5.0 时 `102 × 0.95 = 96.9 < 99` → stop = range_low = 99.0。
 ...
 步骤 6 把 Decimal 距离转 float 再做地板除(注释:与 SEPA 定尺寸一致)。
原行为:float64 算术;改进方案:全程 Decimal。Go 默认照抄 float64 路径以保证股数一致。

### 3.6 出场 `_maybe_exit(bar, local_ts)`
 ...
 **TimeInForce=DAY**(日内专属; 其余策略用 GTC)。
...
- API 意图模式:`strategy_id="intraday_breakout"`,
...
3. **格式化**:`{x:+.2f}`(带符号两位小数,Go: `fmt.Sprintf("%+.2f", x)`);
 `{x:.0f}`(round-half-even;Go `%.0f` 同为 half-even,一致);float 直插
 (`{vol_multiple}`)按最短表示(1.5→"1.5", 2.0→"2.0";Go: `strconv.FormatFloat(x,'g',-1,64)` 时 2.0
 渲染为 "2" —— 需特判,见 Open questions)。
4. **日期/时间**:日期 ISO `YYYY-MM-DD`;UTC datetime 序列化经 `json.dumps(default=str)`
 即 datetime 的字符串形式(`2024-01-08 14:30:00+00:00`,注意是空格分隔而非 "T")
...
1. **sector_rotation 的 `timezone` 参数完全未使用**(月翻转用 UTC 日期)。Go 是否保留这个"死参数"以维持 state_dict/config 形状?
 本规格默认:保留字段、保持未使用(固定形状),另以开关提供
 本地时区日期判定。
2. **月翻转只比较 month 数字**:恰好相隔 12 个月的数据缺口不触发
 再平衡。这是有意为之还是疏漏?默认按原样复刻。
3. **`old <= 0` 的不一致**:`_compute_rebalance_signals` 把该符号剔除出排名,`evaluate_intent` 则记 0.0 回报参与排名。
 两处行为都需照抄,但同一符号在两个视图中的 rank 可能不同——UI 与交易可能短暂不一致。
4. **ORB 盘前 bar 污染区间**:任何 `local_ts < range_end` 的 bar(含 09:30 前)都计入
 开盘区间。生产数据源是否保证只推盘中 bar?若 Go 数据管道含
 盘前数据,需决定开关默认值。
5. **bar.ts 的开盘/收盘时间语义**:源码注释说明若部署方用
 收盘时间戳,边界 bar 被保守地排除;"如果想包含请改 `<=`"。Go 数据层用哪种时间戳
 约定?需要在数据摄入规格中钉死,否则区间组成会差一根 bar。
6. **reason 字符串中 float 的渲染**:最短表示要求 `2.0` → `"2.0"`;Go `%g` 会渲染
 成 `"2"`。若验收以 reason 字符串逐字节比对,需实现强制保留至少一位小数的 float
 渲染(如 strconv 'f' 与最短表示组合)。是否把 reason 比对纳入 gate?
...
8. **ORB 的 hyperopt 接入**:JSON 声明了 search 范围,但 的
