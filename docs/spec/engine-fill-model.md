...
The discovered, empirically-verified rule for **when** a market order fills and
at **what price(s)** in the reference Nautilus `BacktestEngine`, and how the Go
engine's `nautilus-compat` fill profile reproduces it EXACTLY. This is the core
of the P2 accuracy gate (locked decision 3).

Nautilus citations are relative to the installed package
(`/Users/byjackchen/codespace/trade-multi-strategies/.venv/lib/python3.12/site-packages/nautilus_trader`),
version **1.226.0**. Go citations are relative to this repo
(`github.com/byjackchen/trade-tms-go`).

Tags:
- **[MUST-MATCH]** — the Go `nautilus-compat` profile replicates this exactly.
- **[IMPROVE]** — the production `realistic` profile deliberately differs (it is
  a more conservative, non-look-ahead model); the parity GATE never runs it.

The rule below was discovered by driving the real `BacktestEngine` over
controlled bars and observing every `OrderFilled` event. The probes are
permanent and re-runnable:

- `tmp/parity/nautilus_parity.py` — the canonical multi-symbol harness.
- `tmp/parity/probe_depthwalk.py` — the small-volume depth-walk probe (writes
...
The reference wiring (`src/runner/backtest_runner.py`), which the gate mirrors:
...
## 2. Fill TIMING — same bar, at the bar's timestamp [MUST-MATCH]
...
## 3. Fill PRICE — the bar-close depth walk [MUST-MATCH]

When the matching engine decomposes a daily bar into ticks for `LAST`-price
matching (`bar_execution`, `engine.pyx`), it posts the bar's **CLOSE** price as
...
### 3.1 `close_tick_vol` — `compute_bar_quarter_sizes` [MUST-MATCH]

The close-tick depth is `compute_bar_quarter_sizes(volume, min_size)`
(`nautilus_trader/model/data.pxd`), with `min_size = 1` share (equity
...
> yields 3 for `volume=3`, but Nautilus assigns 1). The Go model implements the
...
### 3.2 Depth-walk golden table [MUST-MATCH]

`tmp/parity/probe_depthwalk.py` drives a synthetic 1-share-increment instrument
over `volume ∈ {1,3,7,8,100}` × `side ∈ {BUY,SELL}` × `qty` straddling
`close_tick_vol`, capturing the exact legs Nautilus produces. Representative
...
`internal/exec/fillmodel_depthwalk_test.go` (`TestNautilusCompatDepthWalkGolden`),
which runs in the default `go test ./...`. Regenerate with `make parity-depthwalk`.

---

## 4. Commission — zero (zero-fee equity) [MUST-MATCH]

`TestInstrumentProvider.equity` has zero maker/taker fees, so every observed
fill reports `commission = "0.00 USD"`. The `nautilus-compat` profile returns
zero commission unconditionally (`NautilusCompatModel.Commission`).

---

## 5. AccountState cadence [MUST-MATCH (settled value); cadence is informational]

Nautilus emits an `AccountState` event per fill plus one initial state **per
instrument** at run start (so a 3-instrument run emits 3–4 initial states at the
first bar's ts, all carrying the starting balance), and occasionally a duplicate
at the same ts/value during multi-fill bars. The balance carried is **cash**
(realized only): for the canonical run it steps
`100000 → 100473 → 101045 → 101264 → 101072.40 → 100872.40 → 100661.80`.

The Go engine emits one initial state and one per fill — a different *count*,
but the **settled balance at each unique timestamp is identical**. The parity
comparator therefore compares the last (settled) balance per unique ts, not the
raw event count. This is the only documented place the two engines differ in
cadence; the values match exactly.

---

## 6. NETTING realized-PnL attribution [MUST-MATCH]
...
`realized_pnl` reset to 0. Consequences verified against the engine:
...
(strategy, symbol) life, matching `portfolio.realized_pnl`.
...
> The Nautilus harness's `positions.json` therefore reads
> `portfolio.realized_pnl(iid)` (the cumulative), NOT the final
> `pos.realized_pnl` snapshot, so the golden compares like-for-like against Go.

---

## 7. Parity gate summary

`make parity` runs three steps and the comparator
(`tmp/parity/compare_engine.py`) with these tolerances:
...
First full run result: **12/12 checks PASS** — every fill price/qty/ts, every
per-ticker cumulative realized PnL, every settled account balance, and all 124
equity-curve points match within tolerance (worst equity `|diff| = 1.5e-11`).

The hermetic Go test `internal/parity/run_test.go` asserts the same goldens
without Python (committed under `testdata/parity/`), so the engine, fill model,
accounting and artifact dumper are regression-guarded in the default test run;
the cross-language `make parity` gate (and `go test -tags parity ./internal/parity/`)
re-derives the goldens from the live Nautilus engine.
