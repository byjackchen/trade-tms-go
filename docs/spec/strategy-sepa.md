...
> 本文档是本库对 SEPA 策略的完整定义，覆盖核心状态机、各纯函数指标模块、评级与
> 建仓逻辑、UI 快照类型，以及参数默认值（`internal/hyperopt/baseline/sepa.json`）。
> 下述规则是本系统的不变式（含边界、NaN 处理、取整、顺序、字符串格式）。凡指出
> "已知弱点"之处，同时给出本库采用的更优行为。
...
```
strategy/sepa/
├── indicators   纯函数技术指标（MA、MA 斜率、滚动高低点）
├── swing        摆动高/低点检测（VCP 内部依赖）
├── trend_template  Trend Template 8 条规则
├── stage        Stan Weinstein 风格 Stage 分类器（1/2/3/4/unknown）
├── vcp          VCP（波动收缩形态）检测器
├── grade        A+/B/skip 评级（决定建仓与分批数）
├── intent       SEPASignalIntent（UI 快照类型）
├── signal       SEPASignalGenerator — 纯逻辑核心状态机
├── runner       单标的引擎包装层
└── universe_runner  多标的 Universe runner（active_set + 订阅上限）
```

分层硬性约束（Eng-D2）：指标、摆动检测、Trend Template、Stage、VCP、评级、
signal、intent 全部为纯逻辑，零交易引擎依赖；引擎适配只存在于 runner 文件。
 核心 SEPA 包不得 import broker/engine 包。

数据流（条目化）：

1. 日线 Bar 流入 `SEPASignalGenerator.on_bar`（）。
2. 外部上下文经 setter 推入：`set_regime` / `set_market_cap` /
 `set_earnings_blackout` / `set_catalyst`（）。
...
| volume | int | 成交量 |
 ...
 时间一律 tz-aware UTC（）。测试夹具用美东收盘
≈ 21:00 UTC（）。
内部 kline 缓冲将 Decimal 转 float、volume 转 int 存储（）。
...
| side | enum `LONG`/`FLAT`/`SHORT` | SHORT 未使用（long-only，） |
...
| stop_price | Decimal/nil | 入场时为新止损；止损退出时为被击穿的止损价 |
 ...
 target-position 语义：runner 将 target_qty 翻译为增量订单
（）。

### 1.3 SEPASignalGeneratorConfig（）

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| symbol | string | 是 |
| equity_provider | `func Decimal` | 是（无默认值） | 每次 sizing 时调用，取**实时**权益（） |
...
| timezone | string | 是 | 声明字段；**逻辑中未使用**，仅存储/序列化（测试断言其等于 `America/New_York`，） |
...
- 构造时**绝不调用** `equity_provider`（其闭包可能尚未就绪，）。

### 1.4 参数默认值与 hyperopt 搜索范围（`internal/hyperopt/baseline/sepa.json`）
...
- 解析顺序：显式 `params_dir` > 环境目录 `TMS_STRATEGY_PARAMS_DIR`（仅当该目录下
 存在 `sepa.json`）> 包内 baseline（）。
- `search` 只允许出现在 numeric 类型（float/int）上，否则加载报错（）。
- hyperopt 取样时参数名带前缀：`sepa.risk_pct` 等（）；float 用
...
### 2.1 ma（）
 ...
 `ma(klines, period)` = `close` 列的简单滚动均值，
...
### 2.4 rolling_high / rolling_low（）

 `high` 列 / `low` 列的 N=252 滚动最大/最小，`min_periods == window`
（窗口不满 → NaN，由调用方做回退，见 §3.2）。

---

## 3. Trend Template — 8 条规则（）
...
| 7 | 高于 52 周低点 ≥30% | `low_52w > 0` 时 `close >= low_52w * (1.0 + 0.30)`；`low_52w <= 0` → **False**（） |
| 8 | 市值下限 | `market_cap_usd >= market_cap_min_usd`（**>=**，等于也过， 用 `min-1` 验证失败） |

`passed` = 全部 8 条 AND（）；
`passing_rules` = 通过条数 0..8（，供 intent 评分用）。
...
### 3.3 诊断字段
 ...
 `TrendTemplateResult` 携带：close、ma50、ma150、ma200、high_52w、
low_52w、market_cap_usd、ma200_uptrend_days（）。
测试基准（）：252 根 100→200 线性上升 →
...
### 4.2 算法（）
 ...
 按以下**顺序**短路求值；返回 `"1" | "2" | "3" | "4" | "unknown"`：
...
测试锚点（）：250 根 100→200 线性 → "2"；200→100 → "4"；
110 根升至 180 + 140 根横盘 → "3"；均值 100、σ=0.5 噪声 → "1"；100 根 → "unknown"。

 第 4 步兜底的 NaN-as-False 行为（）在 len<399 时
...
测试锚点（）：11 根 `[100..104,110,104..100]`、lookback=5 →
...
7. **pivot**（）：`last_low_idx` 之后（**不含**该 low 那根）的
 `high` 列最大值；若其后无 bar，回退为整个 klines 最后一根的 high。
8. **底部长度**（）：`base_len = len(klines) - 1 - tail[0].low_idx`
...
12. **输出取整**：contractions 各深度 `round(x, 2)`（round-half-even,
 banker's rounding）；last_contraction_pct `round(.,2)`；
 quality_score `round(.,2)`；vol_dryup_ratio `round(.,3)`；pivot_price 不取整。

 `VCPSnapshot` 字段（）：code、contractions
...
- `earnings_pass = !earnings_blackout`（调用方取反，）。

 regime=="unknown"（Actor 尚未推送）时允许 B 级入场是宽松行为
（ SG 默认 `_regime = "unknown"`，；注意
 注释声称默认 "neutral"，与代码不符）。原行为：
unknown 不拦截。建议改进：Go 增加配置 `requireKnownRegime`（默认 false=原行为），
true 时 unknown/空 regime 视同 skip。

---

## 8. SEPASignalGenerator 状态机（）
...
1. `bar.symbol != config.symbol` → 返回 `[]`，**不追加历史**（）。
2. 追加 bar 到 `_klines`（§8.7）。
3. `_position == 0` → `_maybe_enter`；否则 → `_maybe_exit`。
 同一根 bar 不会既入场又出场。

### 8.3 入场判定链 `_maybe_enter`（）

 按序短路，任一失败返回 `[]`：

1. **Warmup 门槛**：`len(_klines) < 200` → 拒（）。
...
4. **VCP**：`prior = _klines.iloc[:-1]`（**剔除当前 bar**——防止突破 bar 自身的
 high 抬高 pivot 导致 `close <= pivot` 恒假， —— 关键
...
分母**排除当日**是刻意行为（：包含当日会被突破量自身抬高
分母）。注意：模块 docstring（）与 JSON 描述写的是"1.5x 50-day
avg"，**代码实际是 60 根**——以代码为准。见 Open Questions Q2。

### 8.5 建仓 `_build_long_entry`（）
...
stop_price=_stop_price}`，confidence 取默认 1.0。
 ...
 P2 范围注记：tranches 仅用作除数——**只买第一批**，后续批次
加仓逻辑不存在（无 R-ladder、无 pyramid）。A+ 实际首批 = full_shares//3，
B = full_shares//2。

### 8.6 头寸计算 `_compute_first_tranche_shares`（）
...
- **无去重**：同一时间戳重复 on_bar 会追加两行。
 ...
 重复时间戳无去重/无乱序检测。原行为：盲目 append。建议改进：
Go 实现可在 ts ≤ 上一根 ts 时记录 warning 并按配置选择 drop/replace（默认保持
append 以对齐回测）。

### 8.8 出场 `_maybe_exit`（）— P2 仅硬止损
...
- 触发后**先暂存** stop/grade，再清零全部头寸状态（_position/_entry/_stop/
 ...
 `stop_price=旧stop}`。
- `close > stop` → `[]`，持仓不变。
- 没有移动止损、没有时间退出、没有强弱卖出（注释言明 P3+，）。
- regime 转 bear、earnings blackout 开启**不会**触发持仓退出——只挡新入场。

 与 `evaluate_intent` 的 STOP_HIT 判定不一致：on_bar 用
`close <= stop`（），intent 用 `close < stop`（严格 <，
）。close == stop 时实际会被平仓但 intent 显示 HOLD。原行为：
两处不同。建议改进：Go 内部统一为 `<=` 并加注释；若做逐位对照需保留两个谓词。

### 8.9 warmup_from_history（）
...
| rs_rank | nil（未实现） |
 ...
 判定顺序（每次调用先 `_intent_generation += 1`；注意 docstring
自称"不变更状态"但 generation 确实自增——，测试断言严格单调
）：
...
态在自然流程中难以出现（测试通过手工 arm pivot 验证，）。
 保持该语义——pivot 不在 intent 路径中即时计算。
...
- 往返不变量：position、stop、entry、len(klines) 全等
 ...
 若 `_klines` 的索引带名字（如 warmup 传入的 df 索引名为 "ts"），
...
`_REGIME_SLOPE_FLAT_PCT=0.0`（）。
 ...
 `compute_regime(spy_history, as_of)`：
...
- EarningsActor 在 SPY 日线心跳上对每个跟踪 ticker 重算；**首次观察必发布**
 （即使 False），之后仅在翻转时发布（）。
- as_of 取心跳 bar 的 `ts_event` 折算 UTC 日期（）。
...
### 9.3 Market cap（）
 ...
 `load_sf1_market_caps`：SHARADAR/SF1，按 `dimension=="MRT"`
...
为默认值（），非 hyperopt 调参对象；config frozen
（）。运行时可用 `_active_cap_override` /
`_subscription_cap_override` 覆盖（）。
...
### 11.5 warmup_ticker（）
 ...
 对未知 instrument / 空 df 为 no-op；否则同时预热 SG
（`warmup_from_history`）与 screener（`screener.warmup(symbol, df)`）。

### 11.6 信号提交

 与单标的版相同的转译；未知 symbol 在触碰 order_factory 之前
静默返回（、）。
...
| # | 防护 | 位置 | 标签 |
|---|---|---|---|
| 1 | 入场 VCP 检测剔除当前 bar（防突破 bar 自定义 pivot） |
| 2 | 突破量能分母剔除当日 |
| 3 | 摆动点需后置 lookback 根确认（首尾各 lookback 根不出点） |
| 4 | compute_regime 的 as_of 预过滤 |
| 5 | warmup 数据必须严格早于运行窗口（调用方契约） |
| 6 | pre-open 用前一日收盘后的 screener 状态选 top-K（bar 分发前触发） |
| 7 | `evaluate_intent` 的 VCP 用**全量** klines（含当前 bar）——与入场路径不同；intent 仅供 UI，非交易判定 | | （刻意差异，勿"修复"） |
...
- 交易分支硬门槛：`len(_klines) >= 200`（）；Stage 实际要求 220；
...
来自 的 happy-path 夹具（261 根：200 根 50→115 线性 + 60 根
VCP 底 + 1 根突破，，`vcp_lookback=4`、risk_pct=1.0、
equity=100k、cap=10B）：
...
逻辑核心为单线程引擎回调，无显式并发/取消处理。Go 实现按验收线要求补齐，
全部属于"增强不减"，不改变信号语义：
...
- 结构化日志字段至少含 symbol、ts、决策链短路点（被哪一关拒绝）——原版只在
 下单时打日志，建议增加 debug 级拒绝原因（：原行为静默拒绝，难排障；
...
1. **`vcp_lookback` 的 JSON 描述错误**：`sepa.json:52` 写"Number of contraction
 ...
 `find_swing_points` 的 lookback，）。Go 按实际语义实现；
 hyperopt 范围 [3,10] 调的是窗宽。是否要修正文案（不影响行为）？
2. **突破量能基线 60 vs 50**：docstring（）与 JSON 描述
 （`sepa.json:46`）均称 50 日均量，代码硬编码 `base_lookback = 60`
 （）。规格按代码（60）为准；是否将 60 提升为可配置参数？
3. **`_breakout_volume_ok` 引用的 trade-minervini 出处**（）不在
 本仓库内，无法核对其 base 窗口究竟是 50 还是 60；按本仓库代码执行。
4. **regime="unknown" 允许 B 级入场**（§7.2 ）：实盘中 Actor 首发布前
 的窗口极短，但回测若不接 RegimeActor 会全程 unknown 而照常入场。Go 默认值
 是否对齐（建议对齐 + 可选严格模式）？
5. ** 注释称 SG 默认 regime="neutral"**，代码实为
 `"unknown"`（）。规格以代码为准。
6. **evaluate_intent 的 BUY 态在自然流程中不可达**（pivot 仅在入场时写入、出场
 ...
 字段级对照？（规格按保留处理。）
7. **`SEPASignalIntent.rs_rank` 恒为 nil**（）——RS ranking 未在
 本仓库 SEPA 路径实现。Go 保留字段、恒 null？
8. **state_dict 的 klines 索引列名脆弱性**（§8.12 ）：是否采纳兼容
 读取（"index"/"ts"）？
9. **screener（）的 top_k / trend_template_count /
...
11. **timezone 配置项无任何行为**（仅声明 + 序列化 + 测试断言值）。是否在 Go 中
 用于日界判定（当前 universe runner 日界用 UTC date，
...
- ：52 处独立标注行为（不含文首约定与本节）
- ：8 处（§4.2 Stage 兜底 NaN、§6.2 bar 计数命名、§6.2 round-half-even、
