# Spec: UI Surface, Runner Modes, EOD Pipeline, CLI — extracted from `trade-multi-strategies`

Source repo: `/Users/byjackchen/codespace/trade-multi-strategies` (Python reference, READ-ONLY).
All citations are `path:line` relative to that repo root. Every behavior is tagged:

- **[MUST-MATCH]** — Go must replicate exactly (values, ordering, edge cases, string formats).
- **[IMPROVE]** — known weakness in the original; both original behavior and the proposed Go improvement are described. The Go implementation MAY do better but MUST NOT silently change wire-visible semantics that the UI/Redis consumers depend on.
...
[MUST-MATCH] Client env resolution: `API_BASE_URL = NEXT_PUBLIC_API_BASE_URL || "http://127.0.0.1:8000"`, `WS_BASE_URL = NEXT_PUBLIC_WS_BASE_URL || "ws://127.0.0.1:8000"` (`src/ui/src/lib/api/client.ts:15-20`). `apiGet` throws `Error("API <path> -> HTTP <status>")` on non-2xx (`client.ts:24-29`).

[MUST-MATCH] Global query defaults: `refetchInterval: 5000`, `staleTime: 2000` (`src/ui/src/app/providers.tsx:13-16`). Theme: `defaultTheme="dark"`, `enableSystem`, class attribute (`providers.tsx:22`).

> Go port note: the host ports for the Go stack are api **18080** and ui **13000** (project ground rule); the Python defaults above (8000/3000) are what the reference uses. Mapping ports is an allowed deployment change; URL-resolution *logic* is [MUST-MATCH].

### A.2 Route map

Nav links (`src/ui/src/components/site-header.tsx:10-17`), in order: `/` (Live), `/watchlist`, `/strategies`, `/backtest`, `/hyperopt`, `/system`. Active-link rule: `/` requires exact match; others use `pathname.startsWith(href)` (`site-header.tsx:43-46`). [MUST-MATCH]
...
Endpoints present in the API but not currently consumed by any page (must still exist in the Go API for parity): `/api/health`, `/api/live/context`, `/api/live/orders`, `/api/live/strategies/{strategy_id}/state`, `/api/backtest/runs/{ts}/account`, `/api/backtest/runs/{ts}/orders`, `/api/backtest/runs/{ts}/positions` (`src/ui/openapi.json` paths). [MUST-MATCH]
...
[MUST-MATCH] WS reconnect (shared `use-websocket-stream.ts:36-71`): on close OR error, `attempts += 1; delay = min(30_000, 500 * 2^min(attempts, 6))` ms, then reconnect; `attempts` resets to 0 on successful open; JSON parse errors on frames are silently swallowed; cleanup cancels timer and closes socket.

[MUST-MATCH] `staleMs` semantics (broker/data/portfolio-health hooks): `Number.POSITIVE_INFINITY` until first frame, else `max(0, now - lastWallMs)` with `now` ticking at 1 Hz.
...
[MUST-MATCH] Strategy registry — frontend source of truth, must match what the live runner registers (`src/ui/src/lib/strategies.ts:18-33`):

| Nautilus `id` | label | `signalStrategyId` (lowercase intent discriminator) |
|---|---|---|
| `SEPA-UNIVERSE-001` | SEPA | `sepa` |
| `SectorRotation-001` | Sector Rotation | `sector_rotation` |
| `Pairs-001` | Pairs | `pairs` |

[IMPROVE] This registry is hardcoded; comment defers auto-discovery via `GET /api/live/strategies` (`strategies.ts:4-6`). Go may add such an endpoint, but the three IDs above are wire-contract defaults and must stay.

**HealthStrip** — five indicators with pure color functions (`components/cockpit/HealthStrip.tsx:42-100`), all [MUST-MATCH]:
...
**AccountStrip** (`AccountStrip.tsx:11-31`): `/api/live/account` poll 5 s; Total = `Number(total_assets)`, Cash = `Number(cash)`; Day P/L `formatUsd(Number(day_pnl))` + ` (xx.xx%)` from `day_pnl_pct * 100` toFixed(2); Halt-headroom `(halt_headroom_pct*100).toFixed(1)%` or red "HALTED". Red border when halted. [MUST-MATCH]

**StrategyCard** (`StrategyCard.tsx:38-77`): per registry entry — strategy-state WS (running dot = any frame received), positions filtered by exact `String(p["strategy_id"]) === strategyId`, signal counts over the signal stream filtered by `signalStrategyId`: tally `state` into `{buy, forming, hold}` (only keys already in the accumulator count). Shows BUY/FORMING counts; "all strategies — no new orders" line when halted. [MUST-MATCH]

**ActionableSignals** (`ActionableSignals.tsx:54-83`): merge REST snapshot + stream (stream wins when `existing.generation <= incoming.generation` — note ties go to the stream here, vs strict `>` in the hooks); filter to `state === "buy" || state === "forming"`; sort: buy before forming, then `symbol.localeCompare`. [MUST-MATCH] (including the `<=` vs `>=` asymmetry between page-merge and hook-dedupe).

**OpenPositionsCompact** (`OpenPositionsCompact.tsx:23-40`): `/api/live/positions/broker` poll 10 s; unrealized P/L formatted `(n>=0?"+":"") + n.toFixed(2)`, non-finite → raw string, null → "—". Error state → "Broker unreachable". [MUST-MATCH]

**SystemEventsLog** (`SystemEventsLog.tsx:12-141`): badge variant by uppercased state: ERROR/FAULT→destructive; STOPPED/DEGRADED/WARNING→secondary; else outline. Timestamp render: `new Date(ts_ns / 1_000_000).toISOString().replace("T"," ").replace("Z","")`, `0`→"—". Compact mode (cockpit) vs full collapsible mode (100-event tail, auto-scroll). [MUST-MATCH]
...
[MUST-MATCH] Row merge algorithm (`watchlist/page.tsx:86-127`): quote map keyed by symbol (snapshot floor, stream wins on `existing.generation <= incoming`); signal map keyed `symbol:strategy_id` (same rule); rows are one-per-symbol — for a symbol with multiple strategies' signals, the signal with the **higher generation** wins the row's signal column; rows sorted by `symbol.localeCompare`.

[MUST-MATCH] State badge classes keyed by signal state: `no_setup, forming, buy, hold, exit, stop_hit` (`watchlist/page.tsx:42-51`); default state when no signal: `"no_setup"`. Columns: Symbol, Last (`quote.last`), %Chg (`change_pct.toFixed(2)%`, green ≥ 0 else red), State badge, Strategy (`signal.strategy_id`), Strength (`strength.toFixed(1)`), Proximity (`proximity_to_trigger_pct.toFixed(2)%`, null → "—"), Session (`quote.market_session`). Disconnect banner when either WS is down.
...
[MUST-MATCH] `EquityCurveChart` (`components/charts/EquityCurveChart.tsx:22-41`): thins to ≤ 120 evenly spaced points via `step = ceil(n/120)`, always appends the true last point if not already included; Y tick `$${(v/1000).toFixed(0)}K`; X tick = first 10 chars of ISO ts.

[MUST-MATCH] Strategy-state discriminator (`components/strategies/discriminate.ts:74-98`), checked in order:
...
There is **no mode switch in the UI code** — the same pages render in all node modes. The differences are entirely data-driven [MUST-MATCH]:

1. Redis namespace: signal and paper modes default to trader_id `PAPER-SMOKE-001`, which matches the API container's `TMS_TRADER_ID` default, so cockpit/watchlist see streams with zero reconfiguration; live mode uses `TMS-LIVE-REAL-001` and the operator must align the API container's `TMS_TRADER_ID` to see real-money streams (`src/runner/live_runner.py:29-41`).
2. In signal mode no exec client exists, so order submission fails at the RiskEngine by design; the broker-positions/account panels still populate via the API's OpenD REST proxy (reflecting whatever the operator trades manually), and `ActionableSignals` (BUY/FORMING) is the operator's manual-trade prompt (`src/runner/live_runner.py:100-106`, `scripts/live_node.py:6-9`).
3. EOD-refreshed signals land in the same streams, so a populated watchlist does not imply a running live node (`src/runner/eod/__init__.py:1-8`).

---

## Part B — Runner modes (`src/runner`)

### B.1 `build_backtest_engine` (`src/runner/backtest_runner.py:35-85`)

[MUST-MATCH] Signature defaults: `trader_id="MULTI-STRAT-001"`, `log_level="WARNING"`, `bypass_logging=False`, `account_type=AccountType.MARGIN` (required for Pairs shorts), `oms_type=OmsType.NETTING` (`backtest_runner.py:40-44`).

[MUST-MATCH] Wiring sequence:
...
Caller responsibilities are explicitly NOT in this function (pre-load bars, construct runners with returned bar types, register actors BEFORE strategies, register strategies, attach Portfolio, `engine.run()`) — `backtest_runner.py:1-17`.

[MUST-MATCH] `bypass_logging=True` exists because the Rust logging subsystem may only init once per process (tests constructing multiple engines); production leaves it False (`backtest_runner.py:55-59`). Go analogue: logger re-init must be safe across multiple engine constructions in one test binary.

### B.2 Full backtest sequence (`scripts/multi_strategy_backtest.py::run_backtest`, lines 361-825)

Defaults: `start="2024-01-02"`, `end="2024-12-31"`, `starting_balance_usd=100_000.0`, `dump=True`, venue `Venue("SIM")` (`multi_strategy_backtest.py:79,361-371,828-831`). CLI: positional `[START] [END]` only (`:828-831`).

[MUST-MATCH] Override validation: reserved keys per strategy — sepa/sector_rotation: `{strategy_id, bar_types}`; pairs: `{strategy_id, bar_types, pairs_spec}`; violation raises `ValueError("reserved override keys for <name>: [...]")` (`:106-126`).

[MUST-MATCH] Exact step order:
...
3. Per-ticker bars in one pull over `[start_dt - 400 days, end_dt]` (calendar days; ≈270 trading days ≥ the 200-bar TrendTemplate/SEPA threshold), split at `pd.Timestamp(start_dt, tz="UTC")` into warmup (`<`) and run (`>=`) slices (`:405-434`). Skip rules, in order: empty df → skip; empty run slice → skip; run slice price-cap overflow → record in `skipped_overflow`; run slice NaN → record in `skipped_nan`. Warmup slice kept only if non-empty AND NaN-free (otherwise lose pre-warm, never crash ingestion).
   - [MUST-MATCH] Price cap `_NAUTILUS_PRICE_MAX = 17_014_118_346_046.0`; overflow = any of open/high/low/close `max() >` cap (`:87,129-136`). Rationale: reverse-split-adjusted micro-caps overflow Nautilus Price.
   - [MUST-MATCH] NaN check covers open/high/low/close/**volume** (`:139-151`).
   - [MUST-MATCH] Bar frame normalization `_bars_to_engine_format` (`:206-221`): long-format (has `date` column) → index = `to_datetime(date).tz_localize("UTC")`, keep `[open,high,low,close,volume]`; already-indexed naive → `tz_localize("UTC")`; tz-aware → pass through columns only.
...
5. SPY: warmup window = `run_start - 500 days` (calendar; ≈340 trading days > RegimeActor's 200-bar need); single pull split at run_start into `(spy_warmup, spy_run)`; missing → `RuntimeError` (`:335-358`). Context preload (`_load_sf1_mrt` + `_load_earnings`) wrapped in try; on RuntimeError → `actors_wired = False` and continue without actors (`:457-470`). `bars_by_ticker["SPY"] = spy_run` only when actors wired.
   - [MUST-MATCH] `_load_sf1_mrt` (`:223-244`): pyarrow dataset scan of `layout.sf1_root` (hive partitioning), filter `ticker in tickers AND dimension == "MRT"`, keep columns `ticker, datekey, dimension, marketcap` (those present); missing root → empty df.
   - [MUST-MATCH] `_load_earnings` (`:246-267`): scan `layout.events_root`, filter ticker set; earnings = rows whose `eventcodes` string split by `"|"` contains `"22"`; rename `date` → `report_date`; missing root / no `eventcodes` column → empty df.
...
7. `stock_tickers_tuple` = bars_by_ticker keys minus `{"SPY", *sector_tickers}` (`:529-532`). Screener = `SEPAScreener(SEPAScreenerConfig(market_cap_lookup=_market_cap_lookup_factory(cache)))`.
   - [MUST-MATCH] `_market_cap_lookup_factory` (`:182-203`): memoized per-ticker; latest SF1 row by `datekey`; missing/NaN/absent `marketcap` column → `0.0` (screener treats 0 as fail-rule-8).
...
[IMPROVE] Equity-curve fallback `[starting, final]` makes Sharpe/Calmar meaningless on 2 points (acknowledged in comments `:626-629`). Go must keep the same fallback values for parity of dump contents, but should log a structured warning.

### B.3 Shared assembly (`src/runner/strategy_assembly.py`)

[MUST-MATCH] Target-agnostic helpers: prefer `target.add_actor` / `target.add_strategy`; else `target.trader.add_actor/add_strategy` (TradingNode facade) (`strategy_assembly.py:71-89`).

[MUST-MATCH] `_build_actors` (`:101-178`) — registration order matters:
...
Tests pin: backtest with all data → exactly 3 actors in order Regime, Fundamentals, Earnings (`tests/runner/test_strategy_assembly.py:65-88`); empty SF1 + empty earnings → 1 actor (`:90-117`); live mode → 6 actors (`:268`); backtest ignores moomoo_client (`:305`); None warmup OK (`:325`).

[MUST-MATCH] `_build_strategies` (`:180-227`): construct SEPAUniverseRunner (bar_types = tuple over `sepa_stocks` order, plus screener), SectorRotationRunner (bar_types over `sector_etfs` order), PairsRunner (bar_types over `pair_tickers = tuple({legs})` — a **set comprehension**, i.e. unordered in Python; see Open Questions — plus `pairs_spec`); registration order SEPA → Sector → Pairs; return `[sepa, sector, pairs]`.

[MUST-MATCH] `_build_portfolio` (`:230-257`): allocations SEPA 0.40 / SectorRotation 0.30 / Pairs 0.20 (10% implicit cash); `RiskConstraintsConfig(max_single_name_pct=0.50, concentration_pct=0.40, daily_loss_halt_pct=0.10)`; then `strategy.set_portfolio_service(portfolio)` on each of the three. Identical config in backtest and live (parity goal noted at `:236-239`).

[MUST-MATCH] `_build_portfolio_health_actor` (`:260-283`): live mode only; subscribes to **1-MINUTE** SPY bars — `BarType.from_str(f"{spy_instrument_id}-1-MINUTE-LAST-EXTERNAL")`, NOT the daily spy_bar_type — so day-P/L + halt indicators tick at minute cadence. Backtest returns `[]` (pinned by `tests/runner/test_strategy_assembly.py:346,404`).

[MUST-MATCH] `build_all` (`:286-351`): `mode="live"` with `moomoo_client is None` raises `ValueError("moomoo_client required for live mode ...")`; sequence = actors → strategies → portfolio → (live) portfolio-health actor appended to actor list; returns `AssemblyResult(actors, strategies, portfolio)`.

[MUST-MATCH] msgspec custom encodings registered at import: `MoomooClient → "<MoomooClient>"`, `SharadarClient → "<SharadarClient>"` (lossy, logging-only) (`:44-50`). Go analogue: actor-config serialization for logs must tolerate client handles.

### B.4 `portfolio_glue` (`src/runner/portfolio_glue.py`)

[MUST-MATCH] `build_snapshot_from_nautilus` (`:29-66`): `nav = Decimal(str(account.balance_total(base_currency)))`; positions aggregated over `cache.positions_open()` keyed `(str(strategy_id), str(symbol))` with `signed = int(pos.signed_qty)` (positive long / negative short), **zero-qty entries skipped**, same-key entries summed; `cash = nav` (simplified — balance_total already nets margin); `realized_pnl_today` / `unrealized_pnl_today` default `Decimal(0)` (daily-loss-halt effectively dormant in backtest until the caller tracks deltas — `portfolio_glue.py:14-18`); `last_close` defensively copied. Pinned by `tests/runner/test_portfolio_glue.py:58-117`.

[MUST-MATCH] `maybe_check_portfolio` (`:69-98`): no portfolio configured → True; approved → True; rejected → log warning exactly `"[Portfolio] REJECTED {strategy_id}/{symbol} ({side} {qty}): {rule_name} — {reason}"` (when log present) and return False; missing logger must not crash.

### B.5 Live node — `build_node` mode switch (`src/runner/live_runner.py:96-177`)

Modes: `Literal["signal", "paper", "live"]` (`:25`).

[MUST-MATCH] Trader-id resolution: `TMS_TRADER_ID` env (stripped, non-empty) wins; else per-mode default: signal→`PAPER-SMOKE-001`, paper→`PAPER-SMOKE-001`, live→`TMS-LIVE-REAL-001` (`:37-41,90-93`; pinned by `tests/runner/test_live_runner_dry.py:55-90`).

[MUST-MATCH] Mode switch:
...
[MUST-MATCH] Redis backing (`:136-170`): enabled iff `TMS_USE_REDIS` env == `"1"` (default "1"); `REDIS_HOST` default `127.0.0.1`, `REDIS_PORT` default `6379` (int). `DatabaseConfig(type="redis", connection_timeout=5, response_timeout=5)`; `CacheConfig(database, encoding="json", flush_on_start=False)`; `MessageBusConfig(database, encoding="json", stream_per_topic=True)`.
...
### B.6 Live node — `build_and_run` warmup flow + auto-catchup (`src/runner/live_runner.py:179-384`)
...
8. moomoo client: prefer `node.kernel.data_engine.get_client("MOOMOO")`; on exception fall back to the `MoomooClient` **class object** (not an instance) with a comment that actor wiring will then fail loudly at build_all (`:336-348` region; see [IMPROVE] below).
...
[MUST-MATCH] `_resolve_universe_limit` (`:50-61`): `TMS_LIVE_UNIVERSE_LIMIT` env stripped; empty → 85 (`_DEFAULT_LIVE_UNIVERSE_LIMIT`, derived from moomoo OpenD's hard cap of 100 simultaneous K-line subscriptions minus ~15 for SPY+ETFs+pair legs); non-int → `ValueError("TMS_LIVE_UNIVERSE_LIMIT must be an integer, got ...")` (fail fast, no silent fallback — pinned `tests/runner/test_live_universe_limit.py:80-97`).

[MUST-MATCH] `_apply_universe_limit` (`:63-87`), pinned by `tests/runner/test_live_universe_limit.py:27-77`:
- `limit <= 0` or empty universe → `()` (including negative).
- `len(universe) <= limit` → pass through **unchanged order** (no reshuffle).
- else `sorted(key=market_cap_lookup, reverse=True)[:limit]` — descending market cap; unknown tickers (lookup 0.0) sort last. Note Python `sorted` is stable: equal-cap tickers keep input order. Go must use a stable sort.

[IMPROVE] moomoo-client fallback assigns the class, not an instance (`live_runner.py:341-348`) — acknowledged in-source as deferred construction that "will fail at build_all time with a clear error". Go should construct a proper client instance or return a typed error immediately instead of passing a type-token through.

[IMPROVE] `today = date.today()` uses the host-local date while EOD uses UTC (`refresh.py:417`); Go should use one timezone (UTC) consistently — behavior difference only matters around midnight local.

### B.7 Auto-catchup detail (`src/data/sharadar_cache/catchup.py::ensure_cache_fresh`, lines 87-216)

[MUST-MATCH]:
- If `meta.last_sync["SEP"]` missing → return `CatchupReport(skipped_reason="not-bootstrapped")` with a warning telling operator to run bootstrap (full backfill is hours, never implicit at startup) (`catchup.py:106-117`).
...
- Re-syncing the last-synced day on every start is intentional; the underlying writers are same-day idempotent (pinned by `tests/data/sharadar_cache/test_writer_sep.py:78-92` et al.).

### B.8 Daily restart (`scripts/restart_live_node.sh`)

[MUST-MATCH] (operationally — Go equivalent must preserve semantics): `set -euo pipefail`; log file `${TMS_LIVE_LOG:-$HOME/Library/Logs/tms-live-node.log}`, grace `${TMS_RESTART_GRACE_SECS:-30}` s; find PIDs via `pgrep -f "scripts/live_node.py"`; SIGTERM all; poll every 2 s up to grace; SIGKILL stragglers + 1 s sleep; then `nohup uv run python scripts/live_node.py >>"$LOG_FILE" 2>&1 &` (survives launchd job exit). Safe when nothing is running. Mode comes from `.env` (`TMS_LIVE_MODE`, default signal). Rationale: in-memory actors read Sharadar warmup once at startup, so a clean restart (not in-process refresh) picks up the fresh cache through auto-catchup (`restart_live_node.sh:7-13`).
...
The live node only emits signals when MOOMOO delivers the daily bar at session close; if the node is down at that moment the watchlist stays empty. The EOD refresher replays the local Sharadar cache through the **same `SignalGenerator.on_bar()` code path** live mode uses (no parallel state machine) and publishes the **same wire schema** to the same Redis streams — the UI cannot distinguish producers (`src/runner/eod/refresh.py:1-24`, `src/runner/eod/__init__.py:1-8`). Pure Python, no Nautilus engine.

### C.2 Sink protocol (`src/runner/eod/sinks.py`)

[MUST-MATCH] Topics (no `*`): `SIGNAL_INTENT_TOPIC = "data.SignalIntentUpdate"`, `STRATEGY_STATE_TOPIC = "data.StrategyStateUpdate"` (`sinks.py:35-36`).

[MUST-MATCH] Stream key: `f"trader-{trader_id}:stream:{topic}*"` — the **literal `*` suffix** matches what Nautilus's msgbus produces for wildcard-topic custom Data; the API's RedisReader already falls back through this form (`sinks.py:125-131`). Field `topic` inside the XADD entry also carries the literal-`*` form (`topic + "*"`).

[MUST-MATCH] Intent serialization: intent must be a dataclass else `TypeError("intent must be a dataclass, got <Type>")`; `json.dumps(asdict(intent), default=str)` (`sinks.py:116-122`).

[MUST-MATCH] `RedisIntentSink.publish_intent` XADD payload (single `payload` JSON field + `topic` field):

```json
{"type": "SignalIntentUpdate", "strategy_id": "<sid>", "symbol": "<sym>",
 "intent_json": "<json string of dataclass fields>",
 "ts_event": "<int ns AS STRING>", "ts_init": "<same string>"}
```

ts stringification is deliberate — Nautilus encodes i64 as JSON strings; RedisReader's `int(entry["ts_event"])` round-trip must be identical for live and EOD producers (`sinks.py:160-168`; pinned exactly by `tests/runner/eod/test_sinks.py:108-141`).

[MUST-MATCH] `RedisStateSink.publish_state` payload: `{"type": "StrategyStateUpdate", "strategy_id", "state_json": json.dumps(state_summary, default=str), "ts_event": str(ts), "ts_init": str(ts)}` (`sinks.py:188-207`; pinned by `test_sinks.py:144-166`). UI parses `state_json` a second time.

[MUST-MATCH] Protocol seam: `SignalIntentSink.publish_intent(strategy_id, symbol, intent, ts_event_ns)` / `StrategyStateSink.publish_state(strategy_id, state_summary, ts_event_ns)`; in-memory doubles capture call dicts for tests (`sinks.py:48-112`). Go: define equivalent interfaces; tests must be able to swap in capturing fakes without Redis.

### C.3 Refresh algorithm (`src/runner/eod/refresh.py::refresh_eod_signals`, lines 393-502)

Defaults [MUST-MATCH] (`refresh.py:61-67,396-407`):
...
| intent-stream strategy ids | lowercase `sepa` / `sector_rotation` / `pairs` (`refresh.py:198,309,375`) |

[MUST-MATCH] Step sequence:
...
4. `_refresh_sepa` (`:137-213`) per ticker, sequential:
   ...
   - `intent = sg.evaluate_intent(as_of=last_bar.ts)`; publish intent with `strategy_id="sepa"` and `ts_event_ns = int(last_bar.ts.timestamp() * 1e9)` (bar time, NOT wall clock — pinned by `tests/runner/eod/test_refresh.py:300-340`); then publish state with `strategy_id=sepa_strategy_id` (`SEPA-UNIVERSE-001`) and the same ts.
...
Pinned end-to-end by `tests/runner/eod/test_refresh.py`: one SEPA intent per universe ticker with bars; one sector intent per ETF; 2 pairs intents per pair (one per leg); state ids include all three Nautilus ids; empty cache → all-zero report and zero sink calls; ts_event between first seeded bar and `today`.

### C.4 CLI entry (`scripts/eod_signal_refresh.py`)

[MUST-MATCH] Args (argparse):
...
### C.6 Known non-idempotency [IMPROVE]

The Makefile help advertises `eod-refresh` as "daily, idempotent" (`Makefile:21`), but the implementation is **append-only**: every run XADDs new entries to the two streams with fresh stream IDs and no dedup, no `MAXLEN` trim at the producer (`sinks.py:160-171,196-207`). Re-running the same day appends duplicate intents/states (same `ts_event`, new entries). Consumers read latest-per-key so reads are effectively idempotent, and a ~10k MAXLEN cap is assumed elsewhere in the system design (`docs/superpowers/specs/2026-05-13-p-ui-v3-ab-quote-signal-watchlist-design.md:390`), but the EOD producer itself never trims. Additional non-idempotent surface: SEPA publishes **one StrategyStateUpdate per ticker**, all labeled `SEPA-UNIVERSE-001` (`refresh.py:204-209`) — the cockpit SEPA card therefore shows whichever ticker's per-symbol state landed last, and `state_summaries_published` for SEPA equals the ticker count rather than 1.
...
### D.1 `scripts/live_node.py` (make `live-node`)

| Arg | Values / default | Behavior |
|---|---|---|
| `--mode` | `signal\|paper\|live`; default None | Precedence: CLI flag > `TMS_LIVE_MODE` env (lowercased, stripped; default `signal`) > signal. Invalid → `ValueError` (`live_node.py:31-48`). [MUST-MATCH] signal is the always-safe default even with password in env. |
...
Mode gates [MUST-MATCH]: live → interactive confirmation: print warning block, read input, strip; anything but literal uppercase `LIVE` → "Aborted by user." + `sys.exit(0)` (`:51-72`); then password. paper → password only. signal → neither. Password = `MOOMOO_TRADE_PASSWORD` env; empty → stderr message + `sys.exit(1)` (`:75-90`). `KeyboardInterrupt` from `build_and_run` → clean message + exit 0.

[IMPROVE] `_install_signal_handlers` (`:93-114`, SIGINT/SIGTERM → `node.stop_async()`) is **defined but never called** from `main()` — dead code; SIGTERM (from restart_live_node.sh) currently relies on Nautilus/default handling. Go must implement real signal handling: context cancellation on SIGINT/SIGTERM, graceful node stop within the restart script's 30 s grace.

### D.2 `scripts/eod_signal_refresh.py` (make `eod-refresh`) — see C.4.

### D.3 `src/cli/sync_sharadar_universe.py` (click group; shim `scripts/sync_sharadar_universe.py`; make `sync-universe SUBCMD='...'`)

[MUST-MATCH] Subcommands:
- `bootstrap --start YYYY-MM-DD --end YYYY-MM-DD [--ticker T]...` — full backfill, ordered steps with meta recording after each: [1/4-labelled but actually 5 steps] TICKERS (`write_tickers`) → SEP bootstrap → SFP bootstrap → SF1 bootstrap (per ticker list) → EVENTS bootstrap; with `--ticker` overrides, SEP/SFP are fetched via ticker+date filter and written through the writers' `_normalize` + per-year partition path (smoke-test path) (`sync_sharadar_universe.py:55-123`). No tickers after TICKERS step → `click.Abort`.
...
- `stats` — prints cache root and table `Dataset | Rows | Last sync` for TICKERS/SEP/SF1/EVENTS (NB: SFP missing from the stats loop — `:169-184`; [IMPROVE] Go should include SFP).

Cache root from `TMS_SHARADAR_CACHE_DIR` else `<repo>/cache/sharadar` (`:1-5`).

### D.4 `scripts/run_hyperopt.py` (make `hyperopt STRATEGY=... N_TRIALS=... WORKERS=...`)

[MUST-MATCH] Args: positional `strategy` ∈ `{sepa, sector_rotation, pairs, joint}`; `--n-trials` 20; `--workers` 1; `--start` `2023-01-01`; `--end` `2024-12-31`; `--runs-dir` `runs/hyperopt`; `--seed` 42; `--walk-forward/--no-walk-forward` default True; `--folds` 5; `--embargo-days` 5; `--no-dump-trials`; `--resume <name>`; `--trial-timeout-sec` 600 (0 → disabled/None); `--promote-best` prints (never writes) the `.env` line `TMS_STRATEGY_PARAMS_DIR=<relative best_params dir>` when the dir exists (`run_hyperopt.py:10-74`).

### D.5 `scripts/multi_strategy_backtest.py` (make `multi-strategy-backtest [START=][END=]`)

Positional `[START] [END]`, defaults `2024-01-02` / `2024-12-31`; runs `run_backtest(verbose=True)` (see B.2).

### D.6 `scripts/live_paper_smoke.py` (make `live-paper-smoke`)

`--mode connect-only` (default, no network) | `manual-test-order` (needs OpenD + `MOOMOO_TRADE_PASSWORD`; connects, unlocks trade context, disconnects) | `sepa` (stub, deferred). Redis backing identical to live_runner (`TMS_USE_REDIS=0` disables) (`live_paper_smoke.py:1-50`). Trader id default `PAPER-SMOKE-001`.

### D.7 Other scripts

- `scripts/api_server_smoke.py` (make `api-smoke`): uvicorn `api.server:app` on `127.0.0.1:8000`, log_level info; env: `REDIS_HOST/REDIS_PORT`, `TMS_TRADER_ID`, `TMS_RUNS_DIR` (`api_server_smoke.py:1-36`).
- `scripts/nautilus_hello.py` (make `hello`): synthetic-bar EMACross sanity backtest; no args.
- `scripts/redis_backing_spike.py`: validates Redis cache/msgbus backing with trader `SPIKE-001`; no args.
- `make moomoo-spike` → `python -m adapters.moomoo.spike` (needs OpenD).
- `scripts/sync_sharadar_universe.py`: pure shim re-exporting the click cli (`:1-12`).
- `scripts/restart_live_node.sh` (make `live-node-restart`): see B.8.

### D.8 Makefile targets (complete; `Makefile:1-141`)

`-include .env` + `export` at top — every target sees `.env` vars automatically [MUST-MATCH for the Go task runner].
...
| `watchlist-smoke` | `pytest tests/e2e/test_watchlist_smoke.py -v -m e2e` (needs API + Redis) |
| `live-node` | prints mode/pre-flight help then runs `scripts/live_node.py` |
...
|---|---|---|
| Allocations | SEPA 0.40 / Sector 0.30 / Pairs 0.20 / cash 0.10 | `strategy_assembly.py:242-244` |
| Risk constraints | single-name 0.50, concentration 0.40, daily-loss halt 0.10 | `strategy_assembly.py:249-251` |
| Trader ids | signal/paper `PAPER-SMOKE-001`, live `TMS-LIVE-REAL-001`, backtest `MULTI-STRAT-001`, spike `SPIKE-001` | `live_runner.py:37-41`, `backtest_runner.py:40` |
| Live/EOD universe cap | 85 (env `TMS_LIVE_UNIVERSE_LIMIT` / `TMS_EOD_UNIVERSE_LIMIT` / `--universe-limit`) | `live_runner.py:47`, `eod_signal_refresh.py:69` |
| Live/EOD warmup window | 730 calendar days (2×365) | `live_runner.py:258`, `refresh.py:66` |
| Backtest warmup | stocks 400 d calendar, SPY 500 d calendar | `multi_strategy_backtest.py:409,346` |
| Price cap | 17_014_118_346_046.0 | `multi_strategy_backtest.py:87` |
| EOD equity stub | Decimal("1000000") | `refresh.py:65` |
| Bar types | `<sym>.<venue>-1-DAY-LAST-EXTERNAL` everywhere; PortfolioHealth uses `-1-MINUTE-LAST-EXTERNAL` SPY | `backtest_runner.py:79`, `strategy_assembly.py:277` |
| Redis | `TMS_USE_REDIS` default on; timeouts 5 s/5 s; json encoding; `flush_on_start=False`; `stream_per_topic=True` | `live_runner.py:149-169` |
...
1. **PairsRunner bar_types ordering** — `pair_tickers = tuple({t for pair in pairs_spec for t in pair})` (`strategy_assembly.py:215`) iterates a Python *set*, so the bar_types tuple order is hash-dependent across runs/process seeds. Does PairsRunner's behavior depend on bar_types order (subscription order could affect first-bar timing)? EOD uses `sorted(...)` for the same legs (`refresh.py:381`). Go should pick **sorted** order deliberately — need confirmation this doesn't change live behavior relative to Python.
2. **Live warmup excludes pair legs from SEPA universe?** EOD excludes `{SPY, sector ETFs, pair legs}` (`refresh.py:425`) but live `build_and_run` only excludes `{SPY, sector ETFs}` (`live_runner.py:283-285`), so KO/PEP/etc. can be in the live SEPA universe but never in the EOD one. Intentional or drift? Go must pick one (spec'd here as: replicate each path as-is until answered).
...
7. **`paper_acc_id = int(env)`** (`live_runner.py:127`) raises an uncaught ValueError on non-numeric input (vs the curated error for missing). Go: same fail-fast but with a typed error message — acceptable?
8. **`stats` subcommand omits SFP** (`sync_sharadar_universe.py:180`) — bug or intentional (SFP added later)? Go spec'd to include SFP under [IMPROVE].
...
11. **`equity_curve` ts sort is lexicographic** on the sampler's ts strings (`multi_strategy_backtest.py:707`); safe only if all ts share one ISO format+timezone. Confirm sampler always emits uniform ISO-8601 UTC.
