...
const STRATEGY_OPTIONS: { value: HyperoptStrategy; label: string }[] = [
  ...
  { value: "joint", label: "Joint (SEPA + Sector + Pairs)" },
];

/** sepa/joint require a stock universe (handlers_hyperopt.go §POST validation). */
function needsUniverse(s: HyperoptStrategy): boolean {
  return s === "sepa" || s === "joint";
}

/**
 * Map a hyperopt strategy to the strategy id whose param schema the search-space
 * preset is drawn from. `joint` has no single schema (each sub-strategy is tuned
 * with its own bounds server-side); we show the joint set is server-derived.
 */
...
export function NewStudyDialog({
  ...
  const [strategy, setStrategy] = useState<HyperoptStrategy>("pairs");
  ...
  return (
    <Dialog
      ...
      footer={
        tracked ? (
          <>
            {tracked.done && completedTs != null ? (
              <Link
                href={`/hyperopt/${completedTs}`}
        ...
        <div className="space-y-4" data-testid="hyperopt-form">
          ...
          {needsUniverse(strategy) ? (
            <div className="space-y-1.5" data-testid="study-universe-section">
              ...
              <p className="text-xs text-muted-foreground">
                {strategy === "joint" ? "Joint" : "SEPA"} requires a stock
                universe. Sector Rotation / Pairs derive instruments from params.
              </p>
            </div>
          ) : (
