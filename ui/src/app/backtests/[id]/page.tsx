...
import { BacktestStatusBadge } from "@/components/backtests/status-badge";
import { MetricCard } from "@/components/backtests/metric-card";
import { EquityPanel } from "@/components/backtests/equity-panel";
...
export default function BacktestDetailPage() {
  ...
  const { data, isLoading, error, refetch } = useBacktest(numericId);
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          bt ? `${bt.start_date} → ${bt.end_date} · ${bt.kind}` : undefined
        }
        data-testid="backtest-detail-header"
        actions={back}
      />
      ...
      <main
        ...
        data-backtest-id={numericId ?? undefined}
      >
        {numericId == null ? (
          <ErrorState
            error={new Error("Invalid backtest id.")}
            data-testid="detail-bad-id"
          />
        ) : isLoading ? (
